 ...
 *  - end_sep - index of path separator after component or end of pattern
                 ...
                 glob_predicate_fn should_recurse)
{
  ...
  smartlist_t *unglobbed_paths = unglob_fragment(pattern, end_sep, unglob);
  if (!unglobbed_paths) {
    return -1;
  }
  ...
  int error_found = 0;
  const char *pattern_after = &pattern[end_sep + is_sep]; // skip separator
  SMARTLIST_FOREACH_BEGIN(unglobbed_paths, char *, current_path) {
    ...
    } else if (!(!*pattern_after && is_sep && file_type != FN_DIR)) {
...
process_glob(const char *pattern, int add_no_glob, unglob_fn unglob,
             ...
             glob_predicate_fn should_recurse)
{
  int i, start_sep = -1, end_sep = -1, glob_found = 0, is_last, is_sep;
  ...
  for (i = 0; pattern[i]; i++) {
    ...
    if (is_sep || is_last) {
      start_sep = end_sep;
      // if last char is not a separator, end_sep is after pattern end
      end_sep = is_last && !is_sep ? i + 1 : i;
      if (glob_found) {
        break;
      }
    }
