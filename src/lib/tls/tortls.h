...
void tls_log_errors(tor_tls_t *tls, int severity, int domain,
                    const char *doing);
int tor_tls_context_init(unsigned flags, crypto_pk_t *client_identity,
...
void tor_tls_get_n_raw_bytes(tor_tls_t *tls, size_t *n_read,
                             size_t *n_written);

int tor_tls_get_buffer_sizes(tor_tls_t *tls, size_t *rbuf_capacity,
...
void tor_tls_log_one_error(tor_tls_t *tls, unsigned long err, int severity,
                           int domain, const char *doing);
#else /* !defined(ENABLE_OPENSSL) */
