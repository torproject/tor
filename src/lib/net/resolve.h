...
MOCK_DECL(int, tor_addr_lookup,
          (const char *name, uint16_t family, struct tor_addr_t *addr_out));
...
MOCK_DECL(STATIC int, tor_addr_lookup_host_impl,
          (const char *name, uint16_t family, struct tor_addr_t *addr));
#endif
