...
const char *process_type_to_string(process_type_t type);

typedef enum {
  /** Pass complete \n-terminated lines to the
   *  callback (including the \n). */
...
typedef void (*process_read_callback_t)(process_t *, char *, size_t);

void process_init(void);
void process_free_all(void);
smartlist_t * process_get_all_processes(void);

process_t *process_new(process_type_t type);
...
char *process_get_name(const process_t *process);

smartlist_t *process_get_arguments(const process_t *process);
smartlist_t *process_get_environment(const process_t *process);
process_type_t process_get_type(const process_t *process);
