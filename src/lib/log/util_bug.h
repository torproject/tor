...
#  define tor_assertf(expr, fmt, ...)                                       \
    STMT_BEGIN                                                              \
      if (ASSERT_PREDICT_LIKELY_(expr)) {                                   \
        ...
        tor_assertion_failed_(SHORT_FILE__, __LINE__, __func__, #expr, fmt, \
                              ##__VA_ARGS__);                               \
        tor_abort_();                                                       \
      }                                                                     \
    STMT_END
...
#  define tor_assertf_nonfatal(cond, fmt, ...)                             \
    STMT_BEGIN                                                             \
      if (ASSERT_PREDICT_UNLIKELY_(cond)) {                                \
        ...
        tor_bug_occurred_(SHORT_FILE__, __LINE__, __func__, #cond, 0, fmt, \
                          ##__VA_ARGS__);                                  \
      }                                                                    \
    STMT_END
...
void tor_assertion_failed_(const char *fname, unsigned int line,
                           ...
                           ...);
void tor_bug_occurred_(const char *fname, unsigned int line, const char *func,
