 ...
 */
ssize_t
crypto_dh_compute_secret(int severity, crypto_dh_t *dh, const char *pubkey,
                         size_t pubkey_len, char *secret_out,
                         size_t secret_bytes_out)
{
  ...
  secret_len = result;
  if (crypto_expand_key_material_TAP(
          secret_tmp, secret_len, (uint8_t *)secret_out, secret_bytes_out) < 0)
    goto error;
