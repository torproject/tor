...
HT_PROTOTYPE(digest256map_impl, digest256map_entry_t, node,
             digest256map_entry_hash, digest256map_entries_eq)
HT_GENERATE2(digest256map_impl, digest256map_entry_t, node,
             digest256map_entry_hash, digest256map_entries_eq, 0.6,
...
#define IMPLEMENT_MAP_FNS(maptype, keytype, prefix)                          \
  ...
  MOCK_IMPL(maptype *, prefix##_new, (void))                                 \
  {                                                                          \
    maptype *result;                                                         \
  ...
  void *prefix##_get(const maptype *map, const keytype key)                  \
  {                                                                          \
  ...
  void *prefix##_set(maptype *map, const keytype key, void *val)             \
  {                                                                          \
    prefix##_entry_t search;                                                 \
    void *oldval;                                                            \
    tor_assert(map);                                                         \
    ...
    HT_FIND_OR_INSERT_(                                                      \
        prefix##_impl, node, prefix##_entry_hash, &(map->head),              \
   ...
   */                                                                        \
  void *prefix##_remove(maptype *map, const keytype key)                     \
  {                                                                          \
    prefix##_entry_t *resolve;                                               \
    prefix##_entry_t search;                                                 \
    void *oldval;                                                            \
  ...
  int prefix##_size(const maptype *map)                                      \
  {                                                                          \
    return HT_SIZE(&map->head);                                              \
  }                                                                          \
                                                                             \
  /** Return true iff <b>map</b> has no entries. */                          \
  int prefix##_isempty(const maptype *map)                                   \
  {                                                                          \
    return HT_EMPTY(&map->head);                                             \
  }                                                                          \
                                                                             \
  /** Assert that <b>map</b> is not corrupt. */                              \
  void prefix##_assert_ok(const maptype *map)                                \
  {                                                                          \
   ...
   * <b>map</b>. */                                                          \
  MOCK_IMPL(void, prefix##_free_, (maptype * map, void (*free_val)(void *))) \
  {                                                                          \
   ...
   */                                                                        \
  prefix##_iter_t *prefix##_iter_init(maptype *map)                          \
  {                                                                          \
    tor_assert(map);                                                         \
   ...
   * its new value. */                                                       \
  prefix##_iter_t *prefix##_iter_next(maptype *map, prefix##_iter_t *iter)   \
  {                                                                          \
    tor_assert(map);                                                         \
    tor_assert(iter);                                                        \
  ...
  prefix##_iter_t *prefix##_iter_next_rmv(maptype *map,                      \
                                          prefix##_iter_t *iter)             \
  {                                                                          \
    prefix##_entry_t *rmv;                                                   \
    tor_assert(map);                                                         \
   ...
   * to by iter. */                                                          \
  void prefix##_iter_get(prefix##_iter_t *iter, const keytype *keyp,         \
                         void **valp)                                        \
  {                                                                          \
    tor_assert(iter);                                                        \
    tor_assert(*iter);                                                       \
