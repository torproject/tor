...
#define DIGEST256MAP_FOREACH_MODIFY(map, keyvar, valtype, valvar)          \
  MAP_FOREACH_MODIFY (digest256map, map, const uint8_t *, keyvar, valtype, \
                      valvar)
...
#define DECLARE_TYPED_DIGESTMAP_FNS(prefix, mapname_t, valtype)              \
  ...
  ATTR_UNUSED static inline mapname_t *prefix##_new(void)                    \
  {                                                                          \
    return (mapname_t *)digestmap_new();                                     \
  }                                                                          \
  ATTR_UNUSED static inline digestmap_t *prefix##_to_digestmap(              \
      mapname_t *map)                                                        \
  {                                                                          \
    return (digestmap_t *)map;                                               \
  }                                                                          \
  ATTR_UNUSED static inline valtype *prefix##_get(mapname_t *map,            \
                                                  const char *key)           \
  {                                                                          \
    return (valtype *)digestmap_get((digestmap_t *)map, key);                \
  }                                                                          \
  ATTR_UNUSED static inline valtype *prefix##_set(                           \
      mapname_t *map, const char *key, valtype *val)                         \
  {                                                                          \
    return (valtype *)digestmap_set((digestmap_t *)map, key, val);           \
  }                                                                          \
  ATTR_UNUSED static inline valtype *prefix##_remove(mapname_t *map,         \
                                                     const char *key)        \
  {                                                                          \
    return (valtype *)digestmap_remove((digestmap_t *)map, key);             \
  }                                                                          \
  ATTR_UNUSED static inline void prefix##_f##ree_(mapname_t *map,            \
                                                  void (*free_val)(void *))  \
  {                                                                          \
    digestmap_free_((digestmap_t *)map, free_val);                           \
  }                                                                          \
  ATTR_UNUSED static inline int prefix##_isempty(mapname_t *map)             \
  {                                                                          \
    return digestmap_isempty((digestmap_t *)map);                            \
  }                                                                          \
  ATTR_UNUSED static inline int prefix##_size(mapname_t *map)                \
  {                                                                          \
    return digestmap_size((digestmap_t *)map);                               \
  }                                                                          \
  ATTR_UNUSED static inline prefix##_iter_t *prefix##_iter_init(             \
      mapname_t *map)                                                        \
  {                                                                          \
    return (prefix##_iter_t *)digestmap_iter_init((digestmap_t *)map);       \
  }                                                                          \
  ATTR_UNUSED static inline prefix##_iter_t *prefix##_iter_next(             \
      mapname_t *map, prefix##_iter_t *iter)                                 \
  {                                                                          \
    return (prefix##_iter_t *)digestmap_iter_next((digestmap_t *)map,        \
                                                  (digestmap_iter_t *)iter); \
  }                                                                          \
  ATTR_UNUSED static inline prefix##_iter_t *prefix##_iter_next_rmv(         \
      mapname_t *map, prefix##_iter_t *iter)                                 \
  {                                                                          \
    return (prefix##_iter_t *)digestmap_iter_next_rmv(                       \
        (digestmap_t *)map, (digestmap_iter_t *)iter);                       \
  }                                                                          \
  ATTR_UNUSED static inline void prefix##_iter_get(                          \
      prefix##_iter_t *iter, const char **keyp, valtype **valp)              \
  {                                                                          \
    void *v;                                                                 \
    digestmap_iter_get((digestmap_iter_t *)iter, keyp, &v);                  \
    *valp = v;                                                               \
  }                                                                          \
  ATTR_UNUSED static inline int prefix##_iter_done(prefix##_iter_t *iter)    \
  {                                                                          \
