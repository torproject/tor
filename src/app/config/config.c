                  ...
                  const unsigned flags)
{
  ...
  for (; ports; ports = ports->next) {
    ...
    if (use_server_options) {
      ...
      if (bind_ipv4_only && !tor_addr_is_v4(&addr)) {
        log_warn(LD_CONFIG, "Could not interpret %sPort address as IPv4",
                 portname);
        goto err;
      }
      if (bind_ipv6_only && !tor_addr_is_v6(&addr)) {
...
{
  return tor_addr_is_v4(&port->addr) ||
         (tor_addr_is_unspec(&port->addr)
...
{
  return tor_addr_is_v6(&port->addr) == AF_INET6 ||
         (tor_addr_is_unspec(&port->addr) == AF_UNSPEC
