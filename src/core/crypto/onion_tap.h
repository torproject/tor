...
#define TAP_ONIONSKIN_CHALLENGE_LEN \
  (PKCS1_OAEP_PADDING_OVERHEAD + CIPHER_KEY_LEN + DH1024_KEY_LEN)
...
int onion_skin_TAP_server_handshake(const char *onion_skin,
                                    ...
                                    char *handshake_reply_out, char *key_out,
                                    size_t key_out_len);

int onion_skin_TAP_client_handshake(struct crypto_dh_t *handshake_state,
                                    const char *handshake_reply, char *key_out,
                                    size_t key_out_len, const char **msg_out);
