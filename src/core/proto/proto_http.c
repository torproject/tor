...
peek_buf_has_http_command(const buf_t *buf)
{
  if (buf_peek_startswith(buf, "CONNECT ") ||
      ...
      buf_peek_startswith(buf, "GET ") || buf_peek_startswith(buf, "POST ") ||
 ...
 */
int
fetch_from_buf_http(buf_t *buf, char **headers_out, size_t max_headerlen,
                    ...
                    int force_complete)
{
  ...
  log_debug(LD_HTTP, "headerlen %d, bodylen %d.", (int)headerlen,
            (int)bodylen);

  if (max_headerlen <= headerlen) {
    log_warn(LD_HTTP, "headerlen %d larger than %d. Failing.", (int)headerlen,
             (int)max_headerlen - 1);
    return -1;
  }
  if (max_bodylen <= bodylen) {
    log_warn(LD_HTTP, "bodylen %d larger than %d. Failing.", (int)bodylen,
             (int)max_bodylen - 1);
    return -1;
  }
