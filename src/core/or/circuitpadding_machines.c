...
setup_burst_state_for_hiding_intro_circuits(circpad_state_t *burst_state)
{
  ...
  /* Figure out the length of the BURST state so that it's randomized. It's a
   * random number of cells between 4 and 20. */
  burst_state->length_dist.type = CIRCPAD_DIST_UNIFORM;
  burst_state->length_dist.param1 = HS_MACHINE_STATE_PADDING_MINIMUM;
  burst_state->length_dist.param2 = HS_MACHINE_STATE_PADDING_MAXIMUM;
   ...
   * 1 millisecond:
   *
   * IP circuit 5 was created at 11:56:59.590
   * Relay cells:
