...
static void
ewma_notify_circ_active(circuitmux_t *cmux, circuitmux_policy_data_t *pol_data,
                        circuit_t *circ,
                        circuitmux_policy_circ_data_t *pol_circ_data);
static void
ewma_notify_circ_inactive(circuitmux_t *cmux,
                          ...
                          circuitmux_policy_circ_data_t *pol_circ_data);
static void ewma_notify_xmit_cells(
...
cell_ewma_get_tick(void)
{
  ...
  int32_t msec_diff =
      monotime_coarse_diff_msec32(&start_of_current_tick, &now);
...
static void
ewma_free_circ_data(circuitmux_t *cmux, circuitmux_policy_data_t *pol_data,
                    circuit_t *circ,
                    circuitmux_policy_circ_data_t *pol_circ_data)
...
static void
ewma_notify_xmit_cells(circuitmux_t *cmux, circuitmux_policy_data_t *pol_data,
                       circuit_t *circ,
                       circuitmux_policy_circ_data_t *pol_circ_data,
                              ...
                              const char **source_msg)
{
  ...
  halflife_ms = networkstatus_get_param(
      ...
      CMUX_PRIORITY_HALFLIFE_MSEC_DEFAULT, CMUX_PRIORITY_HALFLIFE_MSEC_MIN,
...
scale_active_circuits(ewma_policy_data_t *pol, unsigned cur_tick)
{
  ...
  factor =
      get_scale_factor(pol->active_circuit_pqueue_last_recalibrated, cur_tick);
