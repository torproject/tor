...
/** Return the first client introduction circuit originating from the global
 * circuit list after <b>start</b> or at the start of the list if <b>start</b>
 * is NULL. Return NULL if no circuit is found.
 *
 * A client introduction point circuit has a purpose of either
 * CIRCUIT_PURPOSE_C_INTRODUCING, CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT or
 * CIRCUIT_PURPOSE_C_INTRODUCE_ACKED. This does not return a circuit marked
 * for close.
 *
 * Contrary to other similar functions, this doesn't look at the circuit
 * state. The goal is to return any client circuit regardless of its state
 * because the caller should decide to keep it or not. */
origin_circuit_t *
circuit_get_next_client_intro_circ(const origin_circuit_t *start)
{
  int idx = 0;
  smartlist_t *lst = circuit_get_global_list();
  ...
  for ( ; idx < smartlist_len(lst); ++idx) {
    ...
    /* Ignore a marked for close circuit or purpose not matching a client
     * intro point or if the state is not open. */
    if (circ->marked_for_close ||
