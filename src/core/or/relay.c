    ...
    case END_STREAM_REASON_EXITPOLICY: {
      ...
      if (rh->length >= 5) {
        ...
        if (rh->length == 5 || rh->length == 9) {
          ...
          if (rh->length == 9)
            ttl =
                (int)ntohl(get_uint32(cell->payload + RELAY_HEADER_SIZE + 5));
        } else if (rh->length == 17 || rh->length == 21) {
          tor_addr_from_ipv6_bytes(
              &addr, (char *)(cell->payload + RELAY_HEADER_SIZE + 1));
          if (rh->length == 21)
            ttl =
                (int)ntohl(get_uint32(cell->payload + RELAY_HEADER_SIZE + 17));
        }
                          ...
                          relay_header_t *rh, int optimistic_data)
{
  ...
  case RELAY_COMMAND_DATA:
    ...
    connection_buf_add((char *)(cell->payload + RELAY_HEADER_SIZE), rh->length,
                       TO_CONN(conn));
  ...
  case RELAY_COMMAND_END:
    ...
    if (conn->base_.type == CONN_TYPE_AP) {
      ...
      if (entry_conn->socks_request &&
        ...
        log_warn(LD_BUG, "open stream hasn't sent socks answer yet? Closing.");
    }
