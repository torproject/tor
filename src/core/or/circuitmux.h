...
struct circuitmux_policy_t {
  ...
  circuitmux_policy_circ_data_t *(*alloc_circ_data)(
      circuitmux_t *cmux, circuitmux_policy_data_t *pol_data, circuit_t *circ,
      cell_direction_t direction, unsigned int cell_count);
  /* Free circuit policy-specific data */
  void (*free_circ_data)(circuitmux_t *cmux,
                         circuitmux_policy_data_t *pol_data, circuit_t *circ,
...
MOCK_DECL(const circuitmux_policy_t *, circuitmux_get_policy,
          (circuitmux_t * cmux));
...
void circuitmux_append_destroy_cell(channel_t *chan, circuitmux_t *cmux,
 ...
 */
typedef HT_HEAD(chanid_circid_muxinfo_map,
                chanid_circid_muxinfo_t) chanid_circid_muxinfo_map_t;
