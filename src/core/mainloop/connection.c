...
connection_listener_new(const struct sockaddr *listensockaddr,
                        socklen_t listensocklen, int type, const char *address,
                        const port_cfg_t *portcfg, int *addr_in_use);
...
conn_state_to_string(int type, int state)
{
  ...
  CASE_ANY_LISTENER_TYPE:
    ...
    case CONN_TYPE_EXT_OR:
      switch (state) {
        ...
        case EXT_OR_CONN_STATE_OPEN:
          return "open";
        case EXT_OR_CONN_STATE_FLUSHING:
          return "flushing final OKAY";
      }
      break;
    case CONN_TYPE_EXIT:
      switch (state) {
        case EXIT_CONN_STATE_RESOLVING:
          return "waiting for dest info";
        case EXIT_CONN_STATE_CONNECTING:
          return "connecting";
        case EXIT_CONN_STATE_OPEN:
          return "open";
        case EXIT_CONN_STATE_RESOLVEFAILED:
  ...
  tor_snprintf(buf, sizeof(buf),
               "unknown state [%d] on unknown [%s] connection", state,
...
control_connection_new(int socket_family)
{
  ...
  connection_init(time(NULL), TO_CONN(control_conn), CONN_TYPE_CONTROL,
                  socket_family);
  return control_conn;
}
  ...
  if (connection_speaks_cells(conn)) {
    ...
    if (or_conn->chan) {
      ...
      log_info(LD_CHANNEL,
               ...
               or_conn, base_chan, base_chan->global_identifier);
          ...
          (connection_t * conn, int line, const char *file))
{
  ...
  if (conn->marked_for_close) {
    log_warn(LD_BUG,
             ...
             " (first at %s:%d)",
             file, line, conn->marked_for_close_file, conn->marked_for_close);
    tor_fragile_assert();
    return;
  }
...
#define WARN_TOO_MANY_CONNS_INTERVAL (6 * 60 * 60)
  ...
  if ((m = rate_limit_log(&last_warned, approx_time()))) {
    ...
    log_warn(LD_NET,
             ...
             "read doc/TUNING for guidance.%s",
             n_conns, m);
    tor_free(m);
                               ...
                               int purpose, const port_cfg_t *port)
{
  ...
  if (get_parent_directory(p) < 0 || p[0] != '/') {
    log_warn(LD_GENERAL,
             ...
             "relative paths for unix sockets.",
             path);
    goto done;
  }
...
static connection_t *
connection_listener_new(const struct sockaddr *listensockaddr,
                        socklen_t socklen, int type, const char *address,
                        const port_cfg_t *port_cfg, int *addr_in_use)
{
  ...
  if (listensockaddr->sa_family == AF_INET ||
    ...
    log_notice(LD_NET, "Opening %s on %s", conn_type_to_string(type),
               fmt_addrport(&addr, usePort));
    ...
    if (! SOCKET_OK(s)) {
      ...
      if (ERRNO_IS_RESOURCE_LIMIT(e)) {
        ...
        log_warn(LD_NET, "Socket creation failed: %s", tor_socket_strerror(e));
      }
      goto err;
    }
    ...
    if (make_socket_reuseable(s) < 0) {
      log_warn(LD_NET, "Error setting SO_REUSEADDR flag on %s: %s",
               conn_type_to_string(type), tor_socket_strerror(errno));
    }
    ...
    if (listensockaddr->sa_family == AF_INET6) {
       ...
       * IPv4 connections. */
      if (setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, (void *)&one,
                     (socklen_t)sizeof(one)) < 0) {
        int e = tor_socket_errno(s);
    ...
    if (check_location_for_unix_socket(options, address,
                                       (type == CONN_TYPE_CONTROL_LISTENER)
                                           ...
                                           : UNIX_SOCKET_PURPOSE_SOCKS_SOCKET,
                                       port_cfg) < 0) {
      goto err;
    }

    log_notice(LD_NET, "Opening %s on %s", conn_type_to_string(type), address);

    tor_addr_make_unspec(&addr);

    if (unlink(address) < 0 && errno != ENOENT) {
      log_warn(LD_NET, "Could not unlink %s: %s", address, strerror(errno));
      goto err;
    }
...
#  ifdef HAVE_PWD_H
    if (options->User) {
      ...
      if (pw == NULL) {
        ...
        goto err;
      } else if (fstat(s, &st) == 0 && st.st_uid == pw->pw_uid &&
                 st.st_gid == pw->pw_gid) {
        /* No change needed */
      } else if (chown(sandbox_intern_string(address), pw->pw_uid,
                       pw->pw_gid) < 0) {
        log_warn(LD_NET, "Unable to chown() %s socket: %s.", address,
                 strerror(errno));
        goto err;
      }
    }
    ...
    if (! getsockopt(s, SOL_SOCKET, SO_ACCEPTCONN, &value, &len)) {
      if (value == 0) {
        log_err(LD_NET,
                ...
                "getsockopt(.,SO_ACCEPTCONN,.) yields 0.",
                address);
        goto err;
      }
    }
#  endif /* !defined(__APPLE__) */
  ...
  log_fn(usePort == gotPort ? LOG_DEBUG : LOG_NOTICE, LD_NET,
         "%s listening on port %u.", conn_type_to_string(type), gotPort);
  ...
  if (conn->socket_family == AF_UNIX) {
    log_notice(LD_NET, "Opened %s on %s", conn_type_to_string(type),
               conn->address);
  } else {
    log_notice(LD_NET, "Opened %s on %s", conn_type_to_string(type),
               fmt_addrport(&addr, gotPort));
  }
  return conn;
                                 ...
                                 int *addr_in_use)
{
  ...
  if (port->is_unix_addr) {
    listensockaddr = (struct sockaddr *)create_unix_sockaddr(
        port->unix_addr, &address, &listensocklen);
  } else {
    listensockaddr = tor_malloc(sizeof(struct sockaddr_storage));
    listensocklen =
        tor_addr_to_sockaddr(&port->addr, real_port, listensockaddr,
                             sizeof(struct sockaddr_storage));
    address = tor_addr_to_str_dup(&port->addr);
  }

  if (listensockaddr) {
    conn = connection_listener_new(listensockaddr, listensocklen, port->type,
                                   address, port, addr_in_use);
...
check_sockaddr(const struct sockaddr *sa, int len, int level)
{
  ...
  if (sa->sa_family == AF_INET) {
    ...
    if (len != sizeof(struct sockaddr_in)) {
      log_fn(level, LD_NET, "Length of address not as expected: %d vs %d", len,
...
check_sockaddr_family_match(sa_family_t got, connection_t *listener)
{
  if (got != listener->socket_family) {
    log_info(LD_BUG,
             ...
             conn_type_to_string(listener->type), (int)listener->socket_family,
             (int)got);
    return -1;
  }
  return 0;
...
connection_handle_listener_read(connection_t *conn, int new_type)
{
  ...
  log_debug(LD_NET, "Connection accepted on socket %d (child of fd %d).",
            (int)news, (int)conn->s);
  ...
  if (make_socket_reuseable(news) < 0) {
    if (tor_socket_errno(news) == EINVAL) {
      ...
      log_warn(LD_NET, "Error setting SO_REUSEADDR flag on %s: %s",
               conn_type_to_string(new_type), tor_socket_strerror(errno));
    }
 ...
 */
MOCK_IMPL(STATIC int, connection_connect_sockaddr,
           ...
           int *socket_error))
{
  ...
  const int proto = (sa->sa_family == AF_INET6 || sa->sa_family == AF_INET)
                        ? IPPROTO_TCP
                        : 0;
  ...
  if (connect(s, sa, sa_len) < 0) {
    ...
    if (! ERRNO_IS_CONN_EINPROGRESS(e)) {
      /* yuck. kill it. */
      *socket_error = e;
      log_info(LD_NET, "connect() to socket failed: %s",
               tor_socket_strerror(e));
      tor_close_socket(s);
      return -1;
    } else {
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
  ...
  if ((must_ipv4 && tor_addr_family(&real_addr) == AF_INET6) ||
    ...
    log_info(LD_BUG, "Outgoing %s connection to %s violated ClientUseIPv%s 0.",
             conn->type == CONN_TYPE_OR ? "OR" : "Dir", fmt_addr(&real_addr),
  ...
  if ((! pref_ipv6 && tor_addr_family(&real_addr) == AF_INET6) ||
    ...
    log_info(LD_NET,
             ...
             "%d).",
             fmt_addr(&real_addr), conn->type == CONN_TYPE_OR ? "OR" : "Dir",
 ...
 **/
const tor_addr_t *
conn_get_outbound_address(sa_family_t family, const or_options_t *options,
                          unsigned int conn_type)
{
  ...
  if (conn_type == CONN_TYPE_EXIT) {
    if (! tor_addr_is_null(
      ...
      ext_addr =
          &options
               ->OutboundBindAddresses[OUTBOUND_ADDR_EXIT_AND_OR][fam_index];
    }
  } else { // All non-exit connections
    if (! tor_addr_is_null(
            &options->OutboundBindAddresses[OUTBOUND_ADDR_OR][fam_index])) {
      ext_addr = &options->OutboundBindAddresses[OUTBOUND_ADDR_OR][fam_index];
    } else if (! tor_addr_is_null(
                   &options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT_AND_OR]
                                                  [fam_index])) {
      ext_addr =
                   ...
                   const tor_addr_t *addr, uint16_t port, int *socket_error)
{
  ...
  if (! tor_addr_is_loopback(addr)) {
    ...
    if (ext_addr) {
      ...
      bind_addr_len = tor_addr_to_sockaddr(
          ext_addr, 0, (struct sockaddr *)&bind_addr_ss, sizeof(bind_addr_ss));
      if (bind_addr_len == 0) {
        log_warn(LD_NET,
  ...
  log_debug(LD_NET, "Connecting to %s:%u.", escaped_safe_str_client(address),
            port);

  return connection_connect_sockaddr(conn, dest_addr, dest_addr_len, bind_addr,
                                     bind_addr_len, socket_error);
}
                        ...
                        int *socket_error)
{
  ...
  if (strlen(socket_path) + 1 > sizeof(dest_addr.sun_path)) {
    log_warn(LD_NET, "Path %s is too long for an AF_UNIX socket\n",
  ...
  log_debug(LD_NET, "Connecting to AF_UNIX socket at %s.",
            escaped_safe_str_client(socket_path));

  return connection_connect_sockaddr(conn, (struct sockaddr *)&dest_addr,
                                     sizeof(dest_addr), NULL, 0, socket_error);
}
    ...
    case PROXY_CONNECT: {
      ...
      if (base64_authenticator) {
        ...
        tor_snprintf(buf, sizeof(buf),
                     ...
                     addrport, addrport, base64_authenticator);
...
connection_read_https_proxy_response(connection_t *conn)
{
  ...
  switch (fetch_from_buf_http(conn->inbuf, &headers, MAX_HEADERS_SIZE, NULL,
                              NULL, 10000, 0)) {
    case -1: /* overflow */
  ...
  if (parse_http_response(headers, &status_code, &date_header, NULL, &reason) <
      0) {
    log_warn(LD_NET,
...
static int
connection_fetch_from_buf_socks_client(connection_t *conn, int state,
                                       char **reason)
{
...
connection_read_proxy_handshake(connection_t *conn)
{
  ...
  switch (conn->proxy_state) {
    ...
    case PROXY_SOCKS4_WANT_CONNECT_OK:
      ret = connection_fetch_from_buf_socks_client(conn, conn->proxy_state,
                                                   &reason);
      if (ret == 1)
        conn->proxy_state = PROXY_CONNECTED;
      break;
 ...
 **/
static int
retry_listener_ports(smartlist_t *old_conns, const smartlist_t *ports,
                     smartlist_t *new_conns, smartlist_t *replacements,
  ...
  SMARTLIST_FOREACH_BEGIN (old_conns, connection_t *, conn) {
    ...
    SMARTLIST_FOREACH_BEGIN (launch, const port_cfg_t *, wanted) {
      ...
      if (wanted->is_unix_addr) {
         ...
         */
        const int port_matches =
            (wanted->port == CFG_AUTO_PORT || port_matches_exact);
...
retry_all_listeners(smartlist_t *new_conns, int close_all_noncontrol)
{
  ...
  if (retry_listener_ports(listeners, get_configured_ports(), new_conns,
                           replacements, close_all_noncontrol) < 0)
    retval = -1;
...
static ssize_t
connection_bucket_get_share(int base, int priority, ssize_t global_bucket_val,
                            ssize_t conn_bucket)
{
  ssize_t at_most;
...
connection_bucket_read_limit(connection_t *conn, time_t now)
{
  ...
  return connection_bucket_get_share(base, priority, global_bucket_val,
                                     conn_bucket);
}
...
connection_bucket_write_limit(connection_t *conn, time_t now)
{
  ...
  if (connection_speaks_cells(conn)) {
    ...
    if (conn->state == OR_CONN_STATE_OPEN)
      conn_bucket =
          MIN(conn_bucket, token_bucket_rw_get_write(&or_conn->bucket));
    base = get_cell_network_size(or_conn->wide_circ_ids);
  }
...
global_write_bucket_low(connection_t *conn, size_t attempt, int priority)
{
  ...
  if (priority == 1) { /* old-style v1 query */
    ...
    const or_options_t *options = get_options();
    size_t can_write =
        (size_t)(smaller_bucket + 2 * (options->RelayBandwidthRate
...
static void
record_num_bytes_transferred_impl(connection_t *conn, time_t now,
                                  size_t num_read, size_t num_written)
{
  ...
  if (conn->type == CONN_TYPE_OR)
    rep_hist_note_or_conn_bytes(conn->global_identifier, num_read, num_written,
                                now);
...
static void
connection_buckets_decrement(connection_t *conn, time_t now, size_t num_read,
                             size_t num_written)
{
...
connection_bucket_init(void)
{
  ...
  token_bucket_rw_init(&global_bucket, (int32_t)options->BandwidthRate,
                       (int32_t)options->BandwidthBurst, now_ts);
  if (options->RelayBandwidthRate) {
    token_bucket_rw_init(&global_relayed_bucket,
                         (int32_t)options->RelayBandwidthRate,
                         (int32_t)options->RelayBandwidthBurst, now_ts);
  } else {
    token_bucket_rw_init(&global_relayed_bucket,
...
{
  token_bucket_rw_adjust(&global_bucket, (int32_t)options->BandwidthRate,
  ...
  if (connection_buf_read_from_socket(conn, &max_to_read, &socket_error) < 0) {
    ...
    if (conn->type == CONN_TYPE_OR) {
      connection_or_notify_error(
          TO_OR_CONN(conn),
          socket_error != 0 ? errno_to_orconn_end_reason(socket_error)
                            : END_OR_CONN_REASON_CONNRESET,
          socket_error != 0 ? tor_socket_strerror(socket_error)
                            : "(unknown, errno was 0)");
    }
    if (CONN_IS_EDGE(conn)) {
  ...
  if (connection_speaks_cells(conn) &&
    ...
    switch (result) {
      ...
      CASE_TOR_TLS_ERROR_ANY_NONIO:
        log_debug(
            LD_NET, "tls error [%s]. breaking (nickname %s, address %s).",
            tor_tls_err_to_string(result),
            or_conn->nickname ? or_conn->nickname : "not set", conn->address);
        return result;
    ...
    int reached_eof = 0;
    CONN_LOG_PROTECT(conn, result = buf_read_from_socket(conn->inbuf, conn->s,
                                                         at_most, &reached_eof,
                                                         socket_error));
    if (reached_eof)
      conn->inbuf_reached_eof = 1;
  ...
  if (n_read > 0) {
    ...
    *max_to_read = at_most - n_read;
    ...
    if (get_options()->TestingEnableConnBwEvent &&
        (conn->type == CONN_TYPE_OR || conn->type == CONN_TYPE_DIR ||
...
int
connection_buf_get_line(connection_t *conn, char *data, size_t *data_len)
{
 ...
 * appropriate. */
int
connection_fetch_from_buf_http(connection_t *conn, char **headers_out,
                               size_t max_headerlen, char **body_out,
                               size_t *body_used, size_t max_bodylen,
                               int force_complete)
{
  return fetch_from_buf_http(conn->inbuf, headers_out, max_headerlen, body_out,
                             body_used, max_bodylen, force_complete);
}
...
update_send_buffer_size(tor_socket_t sock)
{
  ...
  if (! WSAIoctl(sock, SIO_IDEAL_SEND_BACKLOG_QUERY, NULL, 0, &isb,
...
connection_handle_write_impl(connection_t *conn, int force)
{
  ...
  if (connection_speaks_cells(conn) &&
    ...
    if (conn->state == OR_CONN_STATE_TLS_HANDSHAKING ||
      ...
      if (connection_tls_continue_handshake(or_conn) < 0) {
        ...
        connection_or_notify_error(or_conn, END_OR_CONN_REASON_MISC,
    ...
    result = buf_flush_to_tls(conn->outbuf, or_conn->tls, max_to_write,
    ...
    CONN_LOG_PROTECT(
        conn, result = buf_flush_to_socket(conn->outbuf, conn->s, max_to_write,
                                           &conn->outbuf_flushlen));
    if (result < 0) {
      if (CONN_IS_EDGE(conn))
        connection_edge_end_errno(TO_EDGE_CONN(conn));
  ...
  if (result > 0) {
    ...
    if (connection_flushed_some(conn) < 0) {
      if (connection_speaks_cells(conn)) {
        connection_or_notify_error(TO_OR_CONN(conn), END_OR_CONN_REASON_MISC,
                                   "Got error back from "
...
connection_write_to_buf_failed(connection_t *conn)
{
  if (CONN_IS_EDGE(conn)) {
    ...
    log_warn(LD_NET, "write_to_buf failed. Closing circuit (fd %d).",
             (int)conn->s);
    ...
    log_warn(LD_NET, "write_to_buf failed. Closing connection (fd %d).",
             (int)conn->s);
    connection_mark_for_close(conn);
  }
}
 ...
 */
MOCK_IMPL(void, connection_write_to_buf_impl_,
 ...
 * for close). */
MOCK_IMPL(connection_t *, connection_get_by_type_addr_port_purpose,
          (int type, const tor_addr_t *addr, uint16_t port, int purpose))
{
  CONN_GET_TEMPLATE(conn,
                    (conn->type == type && tor_addr_eq(&conn->addr, addr) &&
                     conn->port == port && conn->purpose == purpose));
}
...
{
  tor_assert(type == CONN_TYPE_DIR || type == CONN_TYPE_AP ||
             type == CONN_TYPE_EXIT);
  tor_assert(rendquery);

  CONN_GET_TEMPLATE(
      conn, (conn->type == type && (! state || state == conn->state)) &&
                ((type == CONN_TYPE_DIR && TO_DIR_CONN(conn)->rend_data &&
                  ! rend_cmp_service_ids(
                      rendquery,
                      rend_data_get_address(TO_DIR_CONN(conn)->rend_data))) ||
                 (CONN_IS_EDGE(conn) && TO_EDGE_CONN(conn)->rend_data &&
                  ! rend_cmp_service_ids(
                      rendquery,
...
#define DIR_CONN_LIST_TEMPLATE(conn_var, conn_test, dirconn_var,        \
                               dirconn_test)                            \
  STMT_BEGIN                                                            \
    smartlist_t *conns = get_connection_array();                        \
    smartlist_t *dir_conns = smartlist_new();                           \
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn_var) {         \
      if (conn_var && (conn_test) && conn_var->type == CONN_TYPE_DIR && \
...
connection_dir_list_by_purpose_and_resource(int purpose, const char *resource)
{
  DIR_CONN_LIST_TEMPLATE(
      conn, conn->purpose == purpose, dirconn,
      0 == strcmp_opt(resource, dirconn->requested_resource));
}
...
{
  DIR_CONN_LIST_TEMPLATE(
      conn, conn->purpose == purpose && conn->state == state, dirconn,
      0 == strcmp_opt(resource, dirconn->requested_resource));
}
...
any_other_active_or_conns(const or_connection_t *this_conn)
{
  ...
  if (conn != NULL) {
    log_debug(LD_DIR, "%s: Found an OR connection: %s", __func__,
              conn->address);
    return 1;
  }
...
client_check_address_changed(tor_socket_t sock)
{
  ...
  SMARTLIST_FOREACH(outgoing_addrs, const tor_addr_t *, a_ptr,
                    if (tor_addr_eq(a_ptr, &out_addr)) return;);
...
oos_victim_comparator(const void **a_v, const void **b_v)
{
  ...
  if (a->type == CONN_TYPE_OR && b->type == CONN_TYPE_OR) {
     ...
     */
    if (a->type == CONN_TYPE_OR)
      return -1;
    else if (b->type == CONN_TYPE_OR)
      return 1;
    else
      return 0;
  }
  ...
  if (smartlist_len(conns) > 0) {
    ...
    for (i = CONN_TYPE_MIN_; i <= CONN_TYPE_MAX_; ++i) {
      ...
      if (conn_counts_by_type[i] > 0) {
        log_info(LD_NET, "%s: %d conns", conn_type_to_string(i),
...
MOCK_IMPL(STATIC void, kill_conn_list_for_oos, (smartlist_t * conns))
{
  ...
  log_notice(LD_NET, "OOS handler marked %d connections",
             smartlist_len(conns));
}
...
{
  if (conn != NULL && (conn->conn_array_index < 0 || conn->marked_for_close)) {
    return 1;
  } else {
    return 0;
  }
...
connection_check_oos(int n_socks, int failed)
{
   ...
   */
  log_debug(LD_NET, "Running the OOS handler (%d open sockets, %s)", n_socks,
            (failed != 0) ? "exhaustion seen" : "no exhaustion");
  ...
  if (n_socks >= get_options()->ConnLimit_high_thresh &&
    ...
    log_notice(LD_NET,
               ...
               n_socks, get_options()->ConnLimit_high_thresh, target_n_socks);
  } else if (failed) {
    /*
  ...
  if (target_n_socks > 0) {
    ...
    if (moribund_socks < n_socks - target_n_socks) {
      ...
      if (conns) {
        ...
        log_notice(LD_NET, "OOS handler killed %d conns",
                   smartlist_len(conns));
...
connection_dump_buffer_mem_stats(int severity)
{
  ...
  tor_log(severity, LD_GENERAL,
          "In buffers for %d connections: %" PRIu64 " used/%" PRIu64
          " allocated",
          smartlist_len(conns), (total_used), (total_alloc));
  for (i = CONN_TYPE_MIN_; i <= CONN_TYPE_MAX_; ++i) {
    if (! n_conns_by_type[i])
      continue;
    tor_log(severity, LD_GENERAL,
            "  For %d %s connections: %" PRIu64 " used/%" PRIu64 " allocated",
            n_conns_by_type[i], conn_type_to_string(i), (used_by_type[i]),
            (alloc_by_type[i]));
  }
}
...
connection_free_all(void)
{
  ...
  SMARTLIST_FOREACH(
      conns, connection_t *, conn,
      if (conn->type == CONN_TYPE_CONTROL) TO_CONTROL_CONN(conn)->event_mask =
          0);
 ...
 * severity.
 */
MOCK_IMPL(void, clock_skew_warning,
          (const connection_t *conn, long apparent_skew, int trusted,
           log_domain_mask_t domain, const char *received, const char *source))
{
  char dbuf[64];
  ...
  log_fn(trusted ? LOG_WARN : LOG_INFO, domain,
         ...
         received, ext_source, apparent_skew > 0 ? "ahead" : "behind", dbuf,
