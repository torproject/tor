...
static int networkstatus_check_required_protocols(const networkstatus_t *ns,
                                                  int client_mode,
                                                  char **warning_out);
static int reload_consensus_from_file(const char *fname, const char *flavor,
                                      unsigned flags, const char *source_dir);
...
networkstatus_reset_download_failures(void)
{
  ...
  log_debug(LD_GENERAL, "In networkstatus_reset_download_failures()");
...
MOCK_IMPL(char *, networkstatus_get_cache_fname,
...
static tor_mmap_t *
networkstatus_map_cached_consensus_impl(int flav, const char *flavorname,
                                        int unverified_consensus)
{
  char *filename =
      networkstatus_get_cache_fname(flav, flavorname, unverified_consensus);
...
router_reload_consensus_networkstatus(void)
{
  ...
  for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
    ...
    reload_consensus_from_file(fname, flavor,
                               flags | NSSET_WAS_WAITING_FOR_CERTS, NULL);
    tor_free(fname);
  }
...
networkstatus_get_voter_by_id(networkstatus_t *vote, const char *identity)
{
  if (! vote || ! vote->voters)
    return NULL;
                                       ...
                                       const authority_cert_t *cert)
{
  ...
  if (crypto_pk_public_checksig(cert->signing_key, signed_digest,
                                signed_digest_len, sig->signature,
...
int
networkstatus_check_consensus_signature(networkstatus_t *consensus, int warn)
{
  int n_good = 0;
  ...
  SMARTLIST_FOREACH_BEGIN (consensus->voters, networkstatus_voter_info_t *,
    ...
    SMARTLIST_FOREACH_BEGIN (voter->sigs, document_signature_t *, sig) {
      if (! sig->good_signature && ! sig->bad_signature && sig->signature) {
  ...
  if (warn > 1 ||
      (warn >= 0 && (n_good + n_missing_key - n_dl_failed_key < n_required))) {
    severity = LOG_WARN;
  } else {
    severity = LOG_INFO;
  }
  ...
  if (warn >= 0) {
    ...
    });
    {
      ...
      smartlist_add_asprintf(
          ...
          "This %s one has %d (%s).",
          n_required, networkstatus_get_flavor_name(consensus->flavor), n_good,
          tmp);
      tor_free(tmp);
      if (n_no_signature) {
        tmp = smartlist_join_strings(list_no_signature, " ", 0, NULL);
        smartlist_add_asprintf(
            sl, "%d (%s) of the authorities we know didn't sign it.",
            n_no_signature, tmp);
        tor_free(tmp);
      }
      if (n_unknown) {
      ...
      if (n_missing_key) {
        smartlist_add_asprintf(sl,
                               ...
                               "because we were missing the keys.",
                               n_missing_key);
      }
...
MOCK_IMPL(const routerstatus_t *, networkstatus_vote_find_entry,
...
int
networkstatus_vote_find_entry_idx(networkstatus_t *ns, const char *digest,
                                  int *found_out)
{
...
router_get_consensus_status_by_descriptor_digest(networkstatus_t *consensus,
                                                 const char *digest)
{
  return router_get_mutable_consensus_status_by_descriptor_digest(consensus,
                                                                  digest);
}
...
MOCK_IMPL(smartlist_t *, router_get_descriptor_digests, (void))
{
  ...
  if (current_ns_consensus) {
    result = router_get_descriptor_digests_in_consensus(current_ns_consensus);
  }
...
MOCK_IMPL(download_status_t *, router_get_dl_status_by_descriptor_digest,
          (const char *d))
{
  routerstatus_t *rs;
  if (! current_ns_consensus)
...
update_consensus_networkstatus_downloads(time_t now)
{
  ...
  for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
    ...
    if (connection_dir_count_by_purpose_and_resource(
            DIR_PURPOSE_FETCH_CONSENSUS, resource) >= max_in_progress_conns) {
      continue;
    }
    ...
    if (we_are_bootstrapping && use_multi_conn &&
      ...
      directory_get_from_dirserver(
          DIR_PURPOSE_FETCH_CONSENSUS, ROUTER_PURPOSE_GENERAL, resource,
          PDS_RETRY_IF_NO_SERVERS, consensus_dl_status[i].want_authority);
    }
  }
}
...
update_consensus_bootstrap_attempt_downloads(
    time_t now, download_status_t *dls,
    download_want_authority_t want_authority)
{
  ...
  while (download_status_is_ready(dls, now)) {
    log_info(LD_DIR,
             ...
             "download.",
             resource,
             (want_authority == DL_WANT_AUTHORITY ? "authority" : "mirror"));
                                              ...
                                              const or_options_t *options)
{
  ...
  if (networkstatus_consensus_can_use_extra_fallbacks(options)) {
    download_status_t *dls_f = &consensus_bootstrap_dl_status
  ...
  if (c && c->valid_after <= now && now <= c->valid_until) {
        ...
        start + crypto_rand_int((int)dl_interval);
    {
      ...
      log_info(LD_DIR,
               ...
               flavor, (c->fresh_until > now) ? "will be" : "was", tbuf1,
               tbuf2, tbuf3);
    }
  } else {
...
{
  return consensus_waiting_for_certs[usable_consensus_flavor()].consensus ? 1
                                                                          : 0;
}
 ...
 */
MOCK_IMPL(download_status_t *, networkstatus_get_dl_status_by_flavor,
          (consensus_flavor_t flavor))
{
  ...
  if ((int)flavor <= N_CONSENSUS_FLAVORS) {
    dl = &((we_are_bootstrapping ? consensus_bootstrap_dl_status
                                 : consensus_dl_status)[flavor]);
  }
...
MOCK_IMPL(download_status_t *, networkstatus_get_dl_status_by_flavor_bootstrap,
          (consensus_flavor_t flavor))
{
  download_status_t *dl = NULL;
...
MOCK_IMPL(download_status_t *, networkstatus_get_dl_status_by_flavor_running,
          (consensus_flavor_t flavor))
{
  download_status_t *dl = NULL;
...
MOCK_IMPL(networkstatus_t *, networkstatus_get_latest_consensus_by_flavor,
 ...
 */
int
networkstatus_valid_after_is_reasonably_live(time_t valid_after, time_t now)
{
 ...
 */
int
networkstatus_valid_until_is_reasonably_live(time_t valid_until, time_t now)
{
...
MOCK_IMPL(networkstatus_t *, networkstatus_get_reasonably_live_consensus,
          (time_t now, int flavor))
{
  networkstatus_t *consensus =
      networkstatus_get_latest_consensus_by_flavor(flavor);
  if (consensus && networkstatus_consensus_reasonably_live(consensus, now))
    return consensus;
  else
    return NULL;
}
 ...
 * we can use them. */
MOCK_IMPL(int, networkstatus_consensus_can_use_extra_fallbacks,
          (const or_options_t *options))
{
...
networkstatus_consensus_is_already_downloading(const char *resource)
{
  ...
  SMARTLIST_FOREACH_BEGIN (fetching_conns, dir_connection_t *, dirconn) {
    ...
    if (base->linked_conn && base->linked_conn->type == CONN_TYPE_AP &&
...
routerstatus_has_changed(const routerstatus_t *a, const routerstatus_t *b)
{
  ...
  return strcmp(a->nickname, b->nickname) ||
         ...
         a->addr != b->addr || a->or_port != b->or_port ||
         a->dir_port != b->dir_port || a->is_authority != b->is_authority ||
         a->is_exit != b->is_exit || a->is_stable != b->is_stable ||
         a->is_fast != b->is_fast ||
         a->is_flagged_running != b->is_flagged_running ||
         a->is_named != b->is_named || a->is_unnamed != b->is_unnamed ||
         a->is_valid != b->is_valid ||
         a->is_possible_guard != b->is_possible_guard ||
         a->is_bad_exit != b->is_bad_exit || a->is_hs_dir != b->is_hs_dir;
                                     ...
                                     const networkstatus_t *new_c)
{
  ...
  SMARTLIST_FOREACH_JOIN(
      ...
      tor_memcmp(rs_old->identity_digest, rs_new->identity_digest, DIGEST_LEN),
                                      ...
                                      const networkstatus_t *old_c)
{
  ...
  SMARTLIST_FOREACH_JOIN(
      old_c->routerstatus_list, routerstatus_t *, rs_old,
 ...
 **/
static int
reload_consensus_from_file(const char *fname, const char *flavor,
                           unsigned flags, const char *source_dir)
{
  ...
  int rv = networkstatus_set_current_consensus(map->data, map->size, flavor,
                                               flags, source_dir);
#ifdef _WIN32
  if (rv < 0 && tor_memstr(map->data, map->size, "\r\n")) {
    ...
    rv = networkstatus_set_current_consensus(content, strlen(content), flavor,
                                             flags, source_dir);
    tor_free(content);
  }
 ...
 */
static void
handle_missing_protocol_warning_impl(const networkstatus_t *c, int is_client)
{
  char *protocol_warning = NULL;

  int should_exit =
      networkstatus_check_required_protocols(c, is_client, &protocol_warning);
  if (protocol_warning) {
    tor_log(should_exit ? LOG_ERR : LOG_WARN, LD_GENERAL, "%s",
            protocol_warning);
  }
  if (should_exit) {
 ...
 */
STATIC void
warn_early_consensus(const networkstatus_t *c, const char *flavor, time_t now)
{
...
networkstatus_set_current_consensus(const char *consensus,
                                    size_t consensus_len, const char *flavor,
                                    unsigned flags, const char *source_dir)
{
  ...
  c = networkstatus_parse_vote_from_string(consensus, consensus_len, NULL,
                                           NS_TYPE_CONSENSUS);
  if (! c) {
      ...
      if (! current_valid_after || c->valid_after > current_valid_after) {
        ...
        if (was_waiting_for_certs && from_cache)
          if (unlink(unverified_fname) != 0) {
            log_debug(LD_FS, "Failed to unlink %s: %s", unverified_fname,
                      strerror(errno));
          }
      }
      goto done;
  ...
  if (we_want_to_fetch_flavor(options, flav)) {
    if (dir_server_mode(get_options())) {
      dirserv_set_cached_consensus_networkstatus(
          consensus, consensus_len, flavor, &c->digests,
          c->digest_sha3_as_signed, c->valid_after);
...
routers_update_all_from_networkstatus(time_t now, int dir_version)
{
  ...
  if (! have_warned_about_old_version) {
    ...
    const char *recommended =
        is_server ? consensus->server_versions : consensus->client_versions;
    ...
    if (status == VS_RECOMMENDED) {
      ...
      if (! have_warned_about_new_version) {
        log_notice(LD_GENERAL,
                   ...
                   VERSION, status == VS_NEW_IN_SERIES ? " in its series" : "",
                   recommended);
      ...
      log_warn(LD_GENERAL,
               ...
               "authorities. Recommended versions are: %s",
               VERSION, status == VS_OLD ? "obsolete" : "not recommended",
               recommended);
                                                   ...
                                                   int reset_failures)
{
  ...
  SMARTLIST_FOREACH_JOIN(ns->routerstatus_list, routerstatus_t *, rs, routers,
                         routerinfo_t *, router,
...
signed_descs_update_status_from_consensus_networkstatus(smartlist_t *descs)
{
  ...
  SMARTLIST_FOREACH(descs, signed_descriptor_t *, d, {
    const routerstatus_t *rs =
        digestmap_get(ns->desc_digest_map, d->signed_descriptor_digest);
    if (rs) {
 ...
 */
void
set_routerstatus_from_routerinfo(routerstatus_t *rs, const node_t *node,
 ...
 * if necessary. */
MOCK_IMPL(int32_t, networkstatus_get_param,
                                    ...
                                    int32_t max_val)
{
  if (torrc_value >= min_val && torrc_value <= max_val)
    ...
    return networkstatus_get_param(ns, param_name, default_val, min_val,
                                   max_val);
}
...
{
  return networkstatus_get_param(ns, "bwweightscale", BW_WEIGHT_SCALE,
                                 BW_MIN_WEIGHT_SCALE, BW_MAX_WEIGHT_SCALE);
}
                            ...
                            int32_t default_val)
{
  ...
  param = get_net_param_from_list(ns->weight_params, weight_name, default_val,
                                  -1, BW_MAX_WEIGHT_SCALE);
  if (param > max) {
    log_warn(LD_DIR,
...
int
getinfo_helper_networkstatus(control_connection_t *conn, const char *question,
                             char **answer, const char **errmsg)
{
  ...
  if (! strcmp(question, "ns/all")) {
    ...
    SMARTLIST_FOREACH(
        networkstatus_get_latest_consensus()->routerstatus_list,
        const routerstatus_t *, rs,
        { smartlist_add(statuses, networkstatus_getinfo_helper_single(rs)); });
    *answer = smartlist_join_strings(statuses, "", 0, NULL);
    SMARTLIST_FOREACH(statuses, char *, cp, tor_free(cp));
...
networkstatus_check_required_protocols(const networkstatus_t *ns,
                                       int client_mode, char **warning_out)
{
