...
HT_GENERATE2(microdesc_map, microdesc_t, node, microdesc_hash_, microdesc_eq_,
...
microdesc_note_outdated_dirserver(const char *relay_digest)
{
  ...
  base16_encode(relay_hexdigest, sizeof(relay_hexdigest), relay_digest,
                DIGEST_LEN);
...
dump_microdescriptor(int fd, microdesc_t *md, size_t *annotation_len_out)
{
  ...
  if (md->last_listed) {
    ...
    if (write_all_to_fd(fd, annotation, strlen(annotation)) < 0) {
      log_warn(LD_DIR, "Couldn't write microdescriptor annotation: %s",
               strerror(errno));
      return -1;
    }
    r += strlen(annotation);
  ...
  if (written != (ssize_t)md->bodylen) {
    ...
    log_warn(LD_DIR,
             ...
             (long)written, (unsigned long)md->bodylen, strerror(errno));
    return -1;
  }
  r += md->bodylen;
  return r;
...
smartlist_t *
microdescs_add_to_cache(microdesc_cache_t *cache, const char *s,
                        const char *eos, saved_location_t where, int no_save,
                        time_t listed_at, smartlist_t *requested_digests256)
{
  ...
  descriptors = microdescs_parse_from_string(s, eos, allow_annotations, where,
                                             invalid_digests);
                             ...
                             int no_save)
{
  ...
  if (where == SAVED_NOWHERE && ! no_save) {
    fd = start_writing_to_file(cache->journal_fname,
                               OPEN_FLAGS_APPEND | O_BINARY, 0600, &open_file);
    if (fd < 0) {
...
microdesc_cache_clear(microdesc_cache_t *cache)
{
  ...
  if (cache->cache_content) {
    ...
    if (res != 0) {
      log_warn(LD_FS, "tor_munmap_file() failed clearing microdesc cache; "
...
microdesc_cache_reload(microdesc_cache_t *cache)
{
  ...
  journal_content =
      read_file_to_str(cache->journal_fname, RFTS_IGNORE_MISSING, &st);
  if (journal_content) {
...
microdesc_cache_clean(microdesc_cache_t *cache, time_t cutoff, int force)
{
    ...
    if (is_old && ! held_by_nodes) {
      ...
      if (is_old) {
        ...
        log_warn(LD_BUG,
                 ...
                 (int)((now - cutoff) / 3600), held_by_nodes,
                 smartlist_len(nodes), networkstatus_age / 3600, ht_badness);

        SMARTLIST_FOREACH_BEGIN (nodes, const node_t *, node) {
          const char *rs_match = "No RS";
          const char *rs_present = "";
          if (node->rs) {
            if (tor_memeq(node->rs->descriptor_digest, (*mdp)->digest,
                          DIGEST256_LEN)) {
          ...
          log_warn(LD_BUG, "  [%d]: ID=%s. md=%p, rs=%p, ri=%p. %s.%s",
                   node_sl_idx, hex_str(node->identity, DIGEST_LEN), node->md,
                   node->rs, node->ri, rs_match, rs_present);
  ...
  if (dropped) {
    log_info(LD_DIR, "Removed %d/%d microdescriptors as old.", dropped,
             dropped + kept);
...
microdesc_cache_rebuild(microdesc_cache_t *cache, int force)
{
  ...
  fd = start_writing_to_file(cache->cache_fname, OPEN_FLAGS_REPLACE | O_BINARY,
                             0600, &open_file);
  if (fd < 0)
    return -1;
  ...
  HT_FOREACH (mdp, microdesc_map, &cache->map) {
    ...
    if (off_real != off) {
      log_warn(LD_BUG,
               ...
               "By my count, I'm at %" PRId64 ", but I should be at %" PRId64,
  ...
  SMARTLIST_FOREACH_BEGIN (wrote, microdesc_t *, md) {
    ...
    if (PREDICT_UNLIKELY(md->bodylen < 9 ||
                         fast_memneq(md->body, "onion-key", 9) != 0)) {
  ...
  if (md->held_in_map) {
    ...
    if (md2 == md) {
      ...
      log_warn(LD_BUG,
               ...
               "set, but microdesc was not in the map.",
               fname, lineno);
    }
    tor_fragile_assert();
  }
                                 ...
                                 int downloadable_only, digest256map_t *skip)
{
  ...
  SMARTLIST_FOREACH_BEGIN (ns->routerstatus_list, routerstatus_t *, rs) {
    if (microdesc_cache_lookup_by_digest256(cache, rs->descriptor_digest))
      continue;
    if (downloadable_only && ! download_status_is_ready(&rs->dl_status, now))
      continue;
...
update_microdesc_downloads(time_t now)
{
  ...
  missing = microdesc_list_missing_digest256(consensus, get_microdesc_cache(),
                                             1, pending);
  digest256map_free(pending, NULL);

  launch_descriptor_downloads(DIR_PURPOSE_FETCH_MICRODESC, missing, NULL, now);
