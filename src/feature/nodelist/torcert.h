...
int tor_cert_checksig(tor_cert_t *cert, const ed25519_public_key_t *pubkey,
                      time_t now);
...
ssize_t tor_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key,
                                       ...
                                       time_t expires, uint8_t **cert);
MOCK_DECL(int, rsa_ed25519_crosscert_check,
...
int or_handshake_certs_rsa_ok(int severity, or_handshake_certs_t *certs,
                              struct tor_tls_t *tls, time_t now);
int or_handshake_certs_ed25519_ok(int severity, or_handshake_certs_t *certs,
                                  struct tor_tls_t *tls, time_t now);
void or_handshake_certs_check_both(int severity, or_handshake_certs_t *certs,
                                   struct tor_tls_t *tls, time_t now,
