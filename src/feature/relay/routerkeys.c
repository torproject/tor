...
#define EXPIRES_SOON(cert, interval) \
  ...
  {
    char *fname = options_get_keydir_fname(options, "ed25519_signing");
    sign = ed_key_init_from_file(
        fname, INIT_ED_KEY_NEEDCERT | INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT,
        LOG_INFO, NULL, 0, 0, CERT_TYPE_ID_SIGNING, &sign_cert, options);
    tor_free(fname);
  ...
  const int need_new_signing_key =
      NULL == use_signing || EXPIRES_SOON(check_signing_cert, 0) ||
  ...
  const int can_make_master_id_key = ! offline_master && NULL == use_signing;
  ...
  {
    uint32_t flags =
        (INIT_ED_KEY_SPLIT | INIT_ED_KEY_EXTRA_STRONG | INIT_ED_KEY_NO_REPAIR);
    ...
    id = ed_key_init_from_file(fname, flags, LOG_WARN, NULL, 0, 0, 0, NULL,
                               options);
    tor_free(fname);
    if (! id) {
      if (need_new_signing_key) {
        ...
        fname =
            options_get_keydir_fname(options, "ed25519_master_id_public_key");
  ...
  if (want_new_signing_key && sign_signing_key_with_id) {
    uint32_t flags =
        (INIT_ED_KEY_CREATE | INIT_ED_KEY_REPLACE | INIT_ED_KEY_EXTRA_STRONG |
         INIT_ED_KEY_NEEDCERT | INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT);
    char *fname = options_get_keydir_fname(options, "ed25519_signing");
    ed25519_keypair_free(sign);
    tor_cert_free(sign_cert);
    sign =
        ed_key_init_from_file(fname, flags, LOG_WARN, sign_signing_key_with_id,
                              now, options->SigningKeyLifetime,
  ...
  if (server_mode(options) &&
    ...
    ssize_t crosscert_len = tor_make_rsa_ed25519_crosscert(
        &id->pubkey, get_server_identity_key(), expiration, &crosscert);
  ...
  if (! current_auth_key || signing_key_changed ||
      EXPIRES_SOON(auth_key_cert, options->TestingAuthKeySlop)) {
    auth = ed_key_new(use_signing, INIT_ED_KEY_NEEDCERT, now,
                      options->TestingAuthKeyLifetime, CERT_TYPE_SIGNING_AUTH,
                      &auth_cert);
 ...
 */
int
generate_ed_link_cert(const or_options_t *options, time_t now, int force)
{
  ...
  link_cert =
      tor_cert_create(get_master_signing_keypair(), CERT_TYPE_SIGNING_LINK,
                      &dummy_key, now, options->TestingLinkCertLifetime, 0);
...
should_make_new_ed_keys(const or_options_t *options, const time_t now)
{
  ...
  if (! fast_memeq(digests->d[DIGEST_SHA256],
                   link_cert_cert->signed_key.pubkey, DIGEST256_LEN)) {
    return 1;
  }
...
#  define MAKECERT(cert, signing, signed_, type, flags)                 \
    cert = tor_cert_create(signing, type, &signed_->pubkey, time(NULL), \
                           86400, flags);                               \
    if (! cert) {                                                       \
  ...
  MAKECERT(signing_key_cert, master_identity_key, master_signing_key,
           CERT_TYPE_ID_SIGNING, CERT_FLAG_INCLUDE_SIGNING_KEY);
  MAKECERT(auth_key_cert, master_signing_key, current_auth_key,
           CERT_TYPE_SIGNING_AUTH, 0);
  ...
  rsa_ed_crosscert_len = tor_make_rsa_ed25519_crosscert(
      &master_identity_key->pubkey, rsa_identity_key, time(NULL) + 86400,
      &rsa_ed_crosscert);
 ...
 */
static void
print_cert_expiration(const char *expiration, const char *description)
{
 ...
 */
static void
log_ed_cert_expiration(const tor_cert_t *cert, const char *description,
                       const char *fname)
{
...
log_master_signing_key_cert_expiration(const or_options_t *options)
{
  ...
  if (signing_key) {
    ...
    log_warn(LD_OR,
             ...
             "we couldn't learn anything about certificate expiration.",
             fn);
  }
...
MOCK_IMPL(const ed25519_keypair_t *, get_master_signing_keypair, (void))
{
  return master_signing_key;
}

MOCK_IMPL(const struct tor_cert_st *, get_master_signing_key_cert, (void))
{
...
void
get_master_rsa_crosscert(const uint8_t **cert_out, size_t *size_out)
{
                              ...
                              time_t now, time_t lifetime, int *sign_out)
{
  ...
  cert = tor_cert_create(&ed_onion_key, CERT_TYPE_ONION_ID, master_id_key, now,
                         lifetime, 0);
                             ...
                             const crypto_pk_t *rsa_id_key, int *len_out)
{
  ...
  *len_out = 0;
  ...
  int r =
      crypto_pk_private_sign(onion_key, (char *)signature, sizeof(signature),
