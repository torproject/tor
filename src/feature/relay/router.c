...
expire_old_onion_keys(void)
{
  ...
  if (file_status(fname) == FN_FILE) {
    if (tor_unlink(fname) != 0) {
      log_warn(LD_FS, "Couldn't unlink old onion key file %s: %s", fname,
               strerror(errno));
    }
  }
  tor_free(fname);
  ...
  if (file_status(fname) == FN_FILE) {
    if (tor_unlink(fname) != 0) {
      log_warn(LD_FS, "Couldn't unlink old ntor onion key file %s: %s", fname,
               strerror(errno));
    }
  }
  tor_free(fname);
...
construct_ntor_key_map(void)
{
  ...
  if (! fast_mem_is_zero((const char *)cur_pk, CURVE25519_PUBKEY_LEN)) {
    dimap_add_entry(
        &m, cur_pk,
        tor_memdup(&curve25519_onion_key, sizeof(curve25519_keypair_t)));
  }
  if (! fast_mem_is_zero((const char *)last_pk, CURVE25519_PUBKEY_LEN) &&
      tor_memneq(cur_pk, last_pk, CURVE25519_PUBKEY_LEN)) {
    dimap_add_entry(
        &m, last_pk,
        tor_memdup(&last_curve25519_onion_key, sizeof(curve25519_keypair_t)));
  }
...
set_server_identity_key(crypto_pk_t *k)
{
  ...
  if (crypto_pk_get_digest(server_identitykey, server_identitykey_digest) <
      0) {
 ...
 */
MOCK_IMPL(crypto_pk_t *, get_server_identity_key, (void))
{
  tor_assert(server_identitykey);
                      ...
                      authority_cert_t **cert_out)
{
  ...
  fname = get_keydir_fname(legacy ? "legacy_signing_key"
  ...
  tor_free(fname);
  fname = get_keydir_fname(legacy ? "legacy_certificate"
...
init_v3_authority_keys(void)
{
  ...
  if (get_options()->V3AuthUseLegacyKey &&
      load_authority_keyset(1, &legacy_signing_key, &legacy_key_certificate) <
          0)
    return -1;
...
get_onion_key_rotation_days_(void)
{
  return networkstatus_get_param(
      NULL, "onion-key-rotation-days", DEFAULT_ONION_KEY_LIFETIME_DAYS,
      MIN_ONION_KEY_LIFETIME_DAYS, MAX_ONION_KEY_LIFETIME_DAYS);
}
...
{
  int grace_period;
  grace_period = networkstatus_get_param(
      NULL, "onion-key-grace-period-days", DEFAULT_ONION_KEY_GRACE_PERIOD_DAYS,
      MIN_ONION_KEY_GRACE_PERIOD_DAYS, get_onion_key_rotation_days_());
...
router_initialize_tls_context(void)
{
  ...
  return tor_tls_context_init(flags, get_tlsclient_identity_key(),
                              server_mode(options) ? get_server_identity_key()
                                                   : NULL,
...
router_write_fingerprint(int hashed)
{
  ...
  const char *fname = hashed ? "hashed-fingerprint" : "fingerprint";
  ...
  if (! hashed) {
    if (crypto_pk_get_fingerprint(get_server_identity_key(), fingerprint, 0) <
        0) {
...
init_keys(void)
{
...

  {
    ...
    r = init_curve25519_keypair_from_file(&curve25519_onion_key, keydir, 1,
                                          LOG_ERR, "onion");
    tor_free(keydir);
    if (r < 0)
      return -1;
    ...
    if (fast_mem_is_zero(
            (const char *)last_curve25519_onion_key.pubkey.public_key,
            CURVE25519_PUBKEY_LEN) &&
        file_status(keydir) == FN_FILE) {
      /* Load keys from non-empty files only.
       * Missing old keys won't be replaced with freshly generated keys. */
      init_curve25519_keypair_from_file(&last_curve25519_onion_key, keydir, 0,
                                        LOG_ERR, "onion");
    }
    tor_free(keydir);
  }
  ...
  type = ((options->V3AuthoritativeDir
               ? (V3_DIRINFO | MICRODESC_DIRINFO | EXTRAINFO_DIRINFO)
               : NO_DIRINFO) |
  ...
  if (! ds) {
    ds = trusted_dir_server_new(options->Nickname, NULL,
                                ...
                                router_get_advertised_or_port(options), NULL,
                                digest, v3_digest, type, 0.0);
    if (! ds) {
      log_err(LD_GENERAL,
  ...
  if (cert) { /* add my own cert to the list of known certs */
    ...
    if (trusted_dirs_load_certs_from_string(
            ...
            TRUSTED_DIRS_CERTS_SRC_SELF, 0, NULL) < 0) {
...
router_should_be_dirserver(const or_options_t *options, int dir_port)
{
  ...
  if (accounting_is_enabled(options) &&
    ...
    log_info(LD_GENERAL,
             ...
             dir_port ? "dirport" : "begindir", effective_bw,
             (options->AccountingMax), interval_length);

    acc_bytes = options->AccountingMax;
    if (get_options()->AccountingRule == ACCT_SUM)
      acc_bytes /= 2;
    if (effective_bw >= acc_bytes / interval_length) {
      new_choice = 0;
 ...
 */
static int
decide_to_advertise_dir_impl(const or_options_t *options, uint16_t dir_port,
...
{
  int port =
      get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, family);
  (void)options;
...
router_upload_dir_desc_to_dirservers(int force)
{
  ...
  directory_post_to_dirservers(
      ...
      (auth & BRIDGE_DIRINFO) ? ROUTER_PURPOSE_BRIDGE : ROUTER_PURPOSE_GENERAL,
...
router_compare_to_my_exit_policy(const tor_addr_t *addr, uint16_t port)
{
  ...
  if ((tor_addr_family(addr) == AF_INET ||
    ...
    return compare_tor_addr_to_addr_policy(addr, port, me->exit_policy) !=
...
router_extrainfo_digest_is_me(const char *digest)
{
  ...
  return tor_memeq(digest, ei->cache_info.signed_descriptor_digest,
                   DIGEST_LEN);
}
 ...
 */
MOCK_IMPL(int, router_pick_published_address,
                                                 ...
                                                 int listener_type)
{
   ...
   */
  int port_v4_cfg =
      get_first_advertised_port_by_type_af(listener_type, AF_INET);
  if (port_v4_cfg != 0 &&
      ! port_exists_by_type_addr32h_port(listener_type, ipv4h_desc_addr,
                                         port_v4_cfg, 1)) {
    const tor_addr_t *port_addr =
        get_first_advertised_addr_by_type_af(listener_type, AF_INET);
    ...
    const char *listener_str =
        (listener_type == CONN_TYPE_OR_LISTENER ? "OR" : "Dir");
    log_warn(LD_CONFIG,
             "The IPv4 %sPort address %s does not match the "
...
get_my_declared_family(const or_options_t *options)
{
  ...
  for (family = options->MyFamily; family; family = family->next) {
    ...
    if (! member) {
      ...
      fp[0] = '$';
      base16_encode(fp + 1, HEX_DIGEST_LEN + 1, member->identity, DIGEST_LEN);
   ...
   * on clients. */
  nodefamily_t *nf = nodefamily_from_members(
      declared_family, router_get_my_id_digest(), NF_WARN_MALFORMED, NULL);
  ...
  if (smartlist_len(result) == 1) {
    ...
    if (singleton[0] == '$') {
      char d[DIGEST_LEN];
      int n =
          base16_decode(d, sizeof(d), singleton + 1, strlen(singleton + 1));
      if (n == DIGEST_LEN &&
 ...
 */
MOCK_IMPL(STATIC int, router_build_fresh_unsigned_routerinfo,
          (routerinfo_t * *ri_out))
{
  ...
  /* For now, at most one IPv6 or-address is being advertised. */
  {
    ...
    SMARTLIST_FOREACH_BEGIN (get_configured_ports(), const port_cfg_t *, p) {
      if (p->type == CONN_TYPE_OR_LISTENER && ! p->server_cfg.no_advertise &&
...
router_dump_and_sign_extrainfo_descriptor_body(extrainfo_t *ei)
{
  ...
  if (extrainfo_dump_to_string(&ei->cache_info.signed_descriptor_body, ei,
  ...
  crypto_digest256((char *)ei->digest256,
                   ...
                   ei->cache_info.signed_descriptor_len, DIGEST_SHA256);
                                        ...
                                        const extrainfo_t *ei)
{
  ...
  memcpy(ri->cache_info.extra_info_digest256, ei->digest256, DIGEST256_LEN);
}
...
router_dump_and_sign_routerinfo_descriptor_body(routerinfo_t *ri)
{
  ...
  if (! (ri->cache_info.signed_descriptor_body = router_dump_router_to_string(
             ri, get_server_identity_key(), get_onion_key(),
 ...
 * <b>prev</b> to <b>cur</b>. */
static void
log_addr_has_changed(int severity, const tor_addr_t *prev,
                     const tor_addr_t *cur, const char *source)
{
  ...
  if (! tor_addr_is_null(prev))
    ...
    log_notice(LD_GENERAL, "Guessed our IP address as %s (source: %s).",
               addrbuf_cur, source);
}
...
{
  tor_snprintf(platform, len, "Tor %s on %s", get_short_version(),
               get_uname());
}
                             ...
                             const ed25519_keypair_t *signing_keypair)
{
  ...
  char *output = NULL;
  const int emit_ed_sigs =
      signing_keypair && router->cache_info.signing_key_cert;
  ...
  if (emit_ed_sigs) {
    ...
    if (base64_encode(
            ed_cert_base64, sizeof(ed_cert_base64),
    ...
    ed25519_public_to_base64(
        ed_fp_base64, &router->cache_info.signing_key_cert->signing_key);
    tor_asprintf(&ed_cert_line,
                 "identity-ed25519\n"
                 "-----BEGIN ED25519 CERT-----\n"
                 "%s"
                 "-----END ED25519 CERT-----\n"
  ...
  if (crypto_pk_write_public_key_to_string(rsa_pubkey, &onion_pkey,
                                           &onion_pkeylen) < 0) {
  ...
  if (crypto_pk_write_public_key_to_string(
          router->identity_pkey, &identity_pkey, &identity_pkeylen) < 0) {
  ...
  if (tap_key && router->cache_info.signing_key_cert &&
    ...
    char buf[256];
    int tap_cc_len = 0;
    uint8_t *tap_cc = make_tap_onion_key_crosscert(
        tap_key, &router->cache_info.signing_key_cert->signing_key,
        router->identity_pkey, &tap_cc_len);
    if (! tap_cc) {
  ...
  if (ntor_keypair && router->cache_info.signing_key_cert &&
    ...
    tor_cert_t *cert = make_ntor_onion_key_crosscert(
        ntor_keypair, &router->cache_info.signing_key_cert->signing_key,
        router->cache_info.published_on, get_onion_key_lifetime(), &sign);
    if (! cert) {
    ...
    if (base64_encode(buf, sizeof(buf), (const char *)cert->encoded,
  ...
  if (router->declared_family && smartlist_len(router->declared_family)) {
    char *family =
        smartlist_join_strings(router->declared_family, " ", 0, NULL);
  ...
  if (router->ipv6_orport && tor_addr_family(&router->ipv6_addr) == AF_INET6) {
    char addr[TOR_ADDR_BUF_LEN];
    const char *a;
    a = tor_addr_to_str(addr, &router->ipv6_addr, sizeof(addr), 1);
    if (a) {
      tor_asprintf(&extra_or_address, "or-address %s:%d\n", a,
                   router->ipv6_orport);
  ...
  smartlist_add_asprintf(
      ...
      "%s%s"
      "%s%s%s",
      router->nickname, address, router->or_port,
      router_should_advertise_dirport(options, router->dir_port),
      ed_cert_line ? ed_cert_line : "",
      extra_or_address ? extra_or_address : "", router->platform, proto_line,
      published, fingerprint, get_uptime(), (int)router->bandwidthrate,
      (int)router->bandwidthburst, (int)router->bandwidthcapacity,
      extra_info_line ? extra_info_line : "",
      (options->DownloadExtraInfo || options->V3AuthoritativeDir)
          ? "caches-extra-info\n"
          : "",
      onion_pkey, identity_pkey, rsa_tap_cc_line ? rsa_tap_cc_line : "",
      ntor_cc_line ? ntor_cc_line : "", family_line,
  ...
  if (router->ipv6_exit_policy) {
    ...
    if (p6 && strcmp(p6, "reject 1-65535")) {
      smartlist_add_asprintf(chunks, "ipv6-policy %s\n", p6);
    }
    tor_free(p6);
  }

  if (router_should_advertise_begindir(
          options, router->supports_tunnelled_dir_requests)) {
  ...
  if (emit_ed_sigs) {
    ...
    crypto_digest_smartlist_prefix(digest, DIGEST256_LEN,
                                   ED_DESC_SIGNATURE_PREFIX, chunks, "",
                                   DIGEST_SHA256);
...
#ifdef DEBUG_ROUTER_DUMP_ROUTER_TO_STRING
  {
    ...
    if (! ri_tmp) {
      log_err(LD_BUG, "We just generated a router descriptor we can't parse.");
                                  ...
                                  int include_ipv6)
{
  ...
  return policy_dump_to_string(router->exit_policy, include_ipv4,
                               include_ipv6);
}
...
extrainfo_dump_to_string_header_helper(
    smartlist_t *chunks, const extrainfo_t *extrainfo,
    const ed25519_keypair_t *signing_keypair, int emit_ed_sigs)
{
  ...
  if (emit_ed_sigs) {
    if (! extrainfo->cache_info.signing_key_cert->signing_key_included ||
        ! ed25519_pubkey_eq(
            &extrainfo->cache_info.signing_key_cert->signed_key,
            &signing_keypair->pubkey)) {
      log_warn(LD_BUG,
  ...
  tor_asprintf(&pre, "extra-info %s %s\n%spublished %s\n", extrainfo->nickname,
               identity, ed_cert_line, published);
                                      ...
                                      int write_stats_to_extrainfo)
{
  ...
  if (options->ExtraInfoStatistics && write_stats_to_extrainfo) {
    ...
    if (options->ExitPortStatistics &&
        load_stats_file("stats" PATH_SEPARATOR "exit-stats", "exit-stats-end",
                        now, &contents) > 0) {
      smartlist_add(chunks, contents);
    }
    if (options->ConnDirectionStatistics &&
        load_stats_file("stats" PATH_SEPARATOR "conn-stats", "conn-bi-direct",
                        now, &contents) > 0) {
      smartlist_add(chunks, contents);
    }
...
extrainfo_dump_to_string_ed_sig_helper(
    smartlist_t *chunks, const ed25519_keypair_t *signing_keypair)
{
                         ...
                         const ed25519_keypair_t *signing_keypair)
{
  ...
  const int emit_ed_sigs =
      signing_keypair && extrainfo->cache_info.signing_key_cert;
  int rv = 0;

  rv = extrainfo_dump_to_string_header_helper(chunks, extrainfo,
                                              signing_keypair, emit_ed_sigs);
  if (rv < 0)
    goto err;
