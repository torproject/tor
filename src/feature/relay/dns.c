...
static void dns_found_answer(const char *address, uint8_t query_type,
                             int dns_answer, const tor_addr_t *addr,
...
evdns_log_cb(int warn, const char *msg)
{
  ...
  if (! strcmpstart(msg, "Nameserver ") &&
      (cp = strstr(msg, " has failed: "))) {
...
static void
cached_resolve_add_answer(cached_resolve_t *resolve, int query_type,
                          int dns_result, const tor_addr_t *answer_addr,
                          const char *answer_hostname, uint32_t ttl)
{
  if (query_type == DNS_PTR) {
    ...
    if (dns_result == DNS_ERR_NONE && answer_addr &&
      ...
      memcpy(&resolve->result_ipv6.addr_ipv6, tor_addr_to_in6(answer_addr),
...
set_expiry(cached_resolve_t *resolve, time_t expires)
{
  ...
  smartlist_pqueue_add(cached_resolve_pqueue,
                       ...
                       offsetof(cached_resolve_t, minheap_idx), resolve);
}
...
purge_expired_resolves(time_t now)
{
  ...
  while (smartlist_len(cached_resolve_pqueue)) {
    ...
    if (resolve->state == CACHE_STATE_PENDING) {
      ...
      log_debug(
          LD_EXIT, "Forgetting old cached resolve (address %s, expires %lu)",
          escaped_safe_str(resolve->address), (unsigned long)resolve->expire);
 ...
 */
MOCK_IMPL(STATIC void, send_resolved_cell,
 ...
 * <b>conn</b>'s attached circuit.
 */
MOCK_IMPL(STATIC void, send_resolved_hostname_cell,
 ...
 */
MOCK_IMPL(STATIC int, dns_resolve_impl,
           ...
           cached_resolve_t **resolve_out))
{
  ...
  *made_connection_pending_out = 0;
   ...
   * connection.
   */
  if ((r = tor_addr_parse_PTR_name(&addr, exitconn->base_.address, AF_UNSPEC,
                                   0)) != 0) {
    if (r == 1) {
      is_reverse = 1;
    ...
    switch (resolve->state) {
      case CACHE_STATE_PENDING:
        ...
        *made_connection_pending_out = 1;
        log_debug(LD_EXIT,
                  "Connection (fd " TOR_SOCKET_T_FORMAT ") waiting "
                  "for pending DNS resolve of %s",
                  exitconn->base_.s,
                  escaped_safe_str(exitconn->base_.address));
        return 0;
      case CACHE_STATE_CACHED:
        log_debug(LD_EXIT,
                  "Connection (fd " TOR_SOCKET_T_FORMAT ") found "
                  "cached answer for %s",
                  exitconn->base_.s, escaped_safe_str(resolve->address));
 ...
 * a successful lookup.
 */
MOCK_IMPL(STATIC int, set_exitconn_info_from_resolve,
           ...
           char **hostname_out))
{
  ...
  ipv6_ok = (resolve->res_status_ipv6 == RES_STATUS_DONE_OK) &&
            (begincell_flags & BEGIN_FLAG_IPV6_OK) && get_options()->IPv6Exit;
  ...
  if (ipv4_ok && ipv6_ok && is_resolve) {
    ...
    if (ipv4_allowed && ! ipv6_allowed) {
       ...
       * prefers */
      answer_with_ipv4 = ! (begincell_flags & BEGIN_FLAG_IPV6_PREFERRED);
    }
  } else {
...
connection_dns_remove(edge_connection_t *conn)
{
  ...
  if (pend->conn == conn) {
    ...
    tor_free(pend);
    log_debug(LD_EXIT,
              "First connection (fd " TOR_SOCKET_T_FORMAT ") no "
    ...
    log_warn(LD_BUG,
             "Connection (fd " TOR_SOCKET_T_FORMAT ") was not waiting "
...
MOCK_IMPL(void, dns_cancel_pending_resolve, (const char *address))
{
  ...
  if (! resolve->pending_connections) {
    log_warn(LD_BUG, "Address %s is pending but has no pending connections!",
             escaped_safe_str(address));
    tor_fragile_assert();
    return;
  }
  ...
  log_debug(LD_EXIT, "Failing all connections waiting on DNS resolve of %s",
            escaped_safe_str(address));
  while (resolve->pending_connections) {
    pend = resolve->pending_connections;
  ...
  if (tmp != resolve) {
    log_err(LD_BUG,
            ...
            " the cache. Tried to purge %s (%p); instead got %s (%p).",
            resolve->address, (void *)resolve, tmp ? tmp->address : "NULL",
            (void *)tmp);
  }
  tor_assert(tmp == resolve);
...
is_test_address(const char *address)
{
  ...
  return options->ServerDNSTestAddresses &&
         smartlist_contains_string_case(options->ServerDNSTestAddresses,
                                        address);
}
 ...
 */
static void
dns_found_answer(const char *address, uint8_t query_type, int dns_answer,
                 const tor_addr_t *addr, const char *hostname, uint32_t ttl)
{
  cached_resolve_t search;
  cached_resolve_t *resolve;
  ...
  cached_resolve_add_answer(resolve, query_type, dns_answer, addr, hostname,
                            ttl);
...
inform_pending_connections(cached_resolve_t *resolve)
{
  ...
  while (resolve->pending_connections) {
    ...
    r = set_exitconn_info_from_resolve(pendconn, resolve, &hostname);
...
make_pending_resolve_cached(cached_resolve_t *resolve)
{
  ...
  {
    cached_resolve_t *new_resolve =
        tor_memdup(resolve, sizeof(cached_resolve_t));
...
configured_nameserver_address(const size_t idx)
{
  ...
  if (evdns_base_get_nameserver_addr(the_evdns_base, (int)idx,
    ...
    if (tor_addr_from_sockaddr(tor_addr, (const struct sockaddr *)&sa, NULL) ==
        0) {
      return tor_addr;
    }
    tor_free(tor_addr);
...
configure_nameservers(int force)
{
  ...
  if (conf_fname) {
    ...
    if (! missing_resolv_conf) {
      ...
      if ((r = evdns_base_resolv_conf_parse(
               the_evdns_base, flags, sandbox_intern_string(conf_fname)))) {
        log_warn(LD_EXIT,
               ...
               void *arg)
{
  ...
  if (type == DNS_IPv6_AAAA) {
    ...
    if (n_ipv6_timeouts > 10 && n_ipv6_timeouts > n_ipv6_requests_made / 2) {
    ...
    if (type == DNS_IPv4_A && count) {
      ...
      if (answer_is_wildcarded(answer_buf)) {
        log_debug(LD_EXIT,
                  ...
                  safe_str(escaped_address), escaped_safe_str(answer_buf));
        was_wildcarded = 1;
        tor_addr_make_unspec(&addr);
      ...
      escaped_address = esc_for_log(string_address);
      log_debug(LD_EXIT, "eventdns said that %s resolves to %s",
                safe_str(escaped_address), escaped_safe_str(hostname));
      tor_free(escaped_address);
    } else if (count) {
      log_info(LD_EXIT,
  ...
  if (result != DNS_ERR_SHUTDOWN)
    dns_found_answer(string_address, orig_query_type, result, &addr, hostname,
                     ttl);
...
{
  const int options =
      get_options()->ServerDNSSearchDomains ? 0 : DNS_QUERY_NO_SEARCH;
  ...
  switch (query_type) {
    case DNS_IPv4_A:
      req = evdns_base_resolve_ipv4(the_evdns_base, address, options,
                                    evdns_callback, addr);
      break;
    case DNS_IPv6_AAAA:
      req = evdns_base_resolve_ipv6(the_evdns_base, address, options,
                                    evdns_callback, addr);
    ...
    case DNS_PTR:
      if (tor_addr_family(ptr_address) == AF_INET)
        req = evdns_base_resolve_reverse(
            the_evdns_base, tor_addr_to_in(ptr_address), DNS_QUERY_NO_SEARCH,
            evdns_callback, addr);
      else if (tor_addr_family(ptr_address) == AF_INET6)
        req = evdns_base_resolve_reverse_ipv6(
            the_evdns_base, tor_addr_to_in6(ptr_address), DNS_QUERY_NO_SEARCH,
            evdns_callback, addr);
      else
        log_warn(LD_BUG,
...
MOCK_IMPL(STATIC int, launch_resolve, (cached_resolve_t * resolve))
{
  ...
  r = tor_addr_parse_PTR_name(&a, resolve->address, AF_UNSPEC, 0);
...
wildcard_increment_answer(const char *id)
{
  ...
  if (*ip > 5 && n_wildcard_requests > 10) {
    if (! dns_wildcard_list)
      dns_wildcard_list = smartlist_new();
    if (! smartlist_contains_string(dns_wildcard_list, id)) {
      tor_log(
          dns_wildcard_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
add_wildcarded_test_address(const char *address)
{
  ...
  n_test_addrs = get_options()->ServerDNSTestAddresses
                     ? smartlist_len(get_options()->ServerDNSTestAddresses)
                     : 0;
                      ...
                      const char *suffix)
{
  ...
  log_info(LD_EXIT,
           ...
           "domains with request for bogus hostname \"%s\"",
           addr);
  ...
  if (is_ipv6)
    req = evdns_base_resolve_ipv6(
        ...
        addr, DNS_QUERY_NO_SEARCH, evdns_wildcard_check_callback,
...
launch_test_addresses(evutil_socket_t fd, short event, void *args)
{
  ...
  if (options->ServerDNSTestAddresses) {
    tor_assert(the_evdns_base);
    SMARTLIST_FOREACH_BEGIN (options->ServerDNSTestAddresses, const char *,
                             address) {
...
dns_launch_correctness_checks(void)
{
  ...
  if (! launch_event)
    launch_event =
        tor_evtimer_new(tor_libevent_get_base(), launch_test_addresses, NULL);
  timeout.tv_sec = 30;
  timeout.tv_usec = 0;
