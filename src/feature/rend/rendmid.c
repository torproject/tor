                                ...
                                size_t request_len)
{
  ...
  log_info(LD_REND, "Received a legacy ESTABLISH_INTRO request on circuit %u",
  ...
  if (crypto_pk_public_checksig_digest(
          pk, (char *)request, 2 + asn1len + DIGEST_LEN,
  ...
  base32_encode(serviceid, REND_SERVICE_ID_LEN_BASE32 + 1, pk_digest,
                REND_SERVICE_ID_LEN);
                          ...
                          size_t request_len)
{
   ...
   */
  if (request_len <
      (DIGEST_LEN + (MAX_NICKNAME_LEN + 1) + REND_COOKIE_LEN + DH1024_KEY_LEN +
       CIPHER_KEY_LEN + PKCS1_OAEP_PADDING_OVERHEAD)) {
    log_warn(LD_PROTOCOL,
  ...
  base32_encode(serviceid, REND_SERVICE_ID_LEN_BASE32 + 1, (char *)request,
                REND_SERVICE_ID_LEN);

  /* The first 20 bytes are all we look at: they have a hash of the service's
   * PK. */
  intro_circ =
      hs_circuitmap_get_intro_circ_v2_relay_side((const uint8_t *)request);
  if (! intro_circ) {
    log_info(LD_REND,
  ...
  if (relay_send_command_from_edge(0, TO_CIRCUIT(intro_circ),
                                   ...
                                   request_len, NULL)) {
    log_warn(LD_GENERAL, "Unable to send INTRODUCE2 cell to Tor client.");
  ...
  if (relay_send_command_from_edge(
          0, TO_CIRCUIT(circ), RELAY_COMMAND_INTRODUCE_ACK, NULL, 0, NULL)) {
...
err:
  ...
  if (relay_send_command_from_edge(0, TO_CIRCUIT(circ),
                                   RELAY_COMMAND_INTRODUCE_ACK, nak_body, 1,
                                   NULL)) {
                              ...
                              size_t request_len)
{
  ...
  if (circ->base_.n_chan) {
    log_warn(LD_PROTOCOL, "Tried to establish rendezvous on non-edge circuit");
    goto err;
  }
  ...
  /* Acknowledge the request. */
  if (relay_send_command_from_edge(0, TO_CIRCUIT(circ),
                                   RELAY_COMMAND_RENDEZVOUS_ESTABLISHED, "", 0,
                                   NULL) < 0) {
                    ...
                    size_t request_len)
{
  ...
  log_info(LD_REND, "Got request for rendezvous from circuit %u to cookie %s.",
           (unsigned)circ->p_circ_id, hexid);
