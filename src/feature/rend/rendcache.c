...
rend_cache_failure_clean(time_t now)
{
  ...
  STRMAP_FOREACH_MODIFY(rend_cache_failure, key, rend_cache_failure_t *, ent)
  {
                             ...
                             const char *service_id)
{
  ...
  SMARTLIST_FOREACH_BEGIN (desc->intro_nodes, rend_intro_point_t *, intro) {
    ...
    const uint8_t *identity = (uint8_t *)intro->extend_info->identity_digest;
...
rend_cache_intro_failure_note(rend_intro_point_failure_t failure,
                              const uint8_t *identity, const char *service_id)
{
  int found;
  rend_cache_failure_intro_t *entry;
...
rend_cache_lookup_v2_desc_as_dir(const char *desc_id, const char **desc)
{
  ...
  if (base32_decode(desc_id_digest, DIGEST_LEN, desc_id,
...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
  ...
  log_info(LD_REND, "Parsed %d and added %d descriptor%s.", number_parsed,
                                   ...
                                   rend_cache_entry_t **entry)
{
  ...
  if (base32_decode(want_desc_id, sizeof(want_desc_id), desc_id_base32,
                    strlen(desc_id_base32)) != sizeof(want_desc_id)) {
  ...
  if (intro_content && intro_size > 0) {
    ...
    if (rend_data->auth_type != REND_NO_AUTH &&
      ...
      if (rend_decrypt_introduction_points(
              &ipos_decrypted, &ipos_decrypted_size,
              rend_data->descriptor_cookie, intro_content, intro_size) < 0) {
        log_warn(LD_REND,
    ...
    n_intro_points =
        rend_parse_introduction_points(parsed, intro_content, intro_size);
    if (n_intro_points <= 0) {
      log_warn(LD_REND,
