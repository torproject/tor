...
rend_client_get_random_intro_impl(const rend_cache_entry_t *rend_query,
...
rend_client_send_establish_rendezvous(origin_circuit_t *circ)
{
  ...
  if (relay_send_command_from_edge(0, TO_CIRCUIT(circ),
                                   ...
                                   REND_COOKIE_LEN, circ->cpath->prev) < 0) {
                              ...
                              origin_circuit_t *rendcirc)
{
  ...
  tor_assert(
      ! rend_cmp_service_ids(rend_data_get_address(introcirc->rend_data),
  ...
  if (r < 0 || ! rend_client_any_intro_points_usable(entry)) {
    ...
    rend_client_refetch_v2_renddesc(introcirc->rend_data);
    {
      ...
      while ((conn = connection_get_by_type_state_rendquery(
                  CONN_TYPE_AP, AP_CONN_STATE_CIRCUIT_WAIT, onion_address))) {
  ...
  if (entry->parsed->protocols & (1 << 3) ||
    ...
    klen = crypto_pk_asn1_encode(
        extend_info->onion_key, tmp + v3_shift + 7 + DIGEST_LEN + 2,
    ...
    memcpy(tmp + v3_shift + 7 + DIGEST_LEN + 2 + klen,
           rendcirc->rend_data->rend_cookie, REND_COOKIE_LEN);
  ...
  r = crypto_pk_obsolete_public_hybrid_encrypt(
      intro_key, payload + DIGEST_LEN, sizeof(payload) - DIGEST_LEN, tmp,
      (int)(dh_offset + DH1024_KEY_LEN), PK_PKCS1_OAEP_PADDING, 0);
  if (r < 0) {
  ...
  if (relay_send_command_from_edge(0, TO_CIRCUIT(introcirc),
                                   RELAY_COMMAND_INTRODUCE1, payload,
                                   payload_len, introcirc->cpath->prev) < 0) {
...
rend_client_close_other_intros(const uint8_t *rend_pk_digest)
{
  ...
  SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, c) {
    if ((c->purpose == CIRCUIT_PURPOSE_C_INTRODUCING ||
      ...
      if (oc->rend_data && rend_circuit_pk_digest_eq(oc, rend_pk_digest)) {
        log_info(LD_REND | LD_CIRC,
                 "Closing introduction circuit %d that we "
                 "built in parallel (Purpose %d).",
                 oc->global_identifier, c->purpose);
 ...
 */
int
rend_client_introduction_acked(origin_circuit_t *circ, const uint8_t *request,
                               size_t request_len)
{
  const or_options_t *options = get_options();
  origin_circuit_t *rendcirc;
  ...
  if (request_len == 0) {
     ...
     */
    log_info(
        LD_REND, "Got nack for %s from %s...",
        safe_str_client(rend_data_get_address(circ->rend_data)),
        safe_str_client(extend_info_describe(circ->build_state->chosen_exit)));
    if (rend_client_report_intro_point_failure(
            circ->build_state->chosen_exit, circ->rend_data,
                          ...
                          routerstatus_t *rs_hsdir)
{
  ...
  if (! rs_hsdir) {
    ...
    if (! hs_dir) {
      ...
      const char *query_response =
          (rate_limited) ? "QUERY_RATE_LIMITED" : "QUERY_NO_HSDIR";
   ...
   * on the control port. */
  hsdir_fp =
      tor_memdup(hs_dir->identity_digest, sizeof(hs_dir->identity_digest));
  ...
  if (rend_data->auth_type != REND_NO_AUTH) {
    if (base64_encode(
            descriptor_cookie_base64, sizeof(descriptor_cookie_base64),
            rend_data->descriptor_cookie, REND_DESC_COOKIE_LEN, 0) < 0) {
  ...
  log_info(LD_REND,
           ...
           "directory %s",
           rend_data->onion_address, desc_id_base32, rend_data->auth_type,
           (rend_data->auth_type == REND_NO_AUTH
                ? "[none]"
                : escaped_safe_str_client(descriptor_cookie_base64)),
           routerstatus_describe(hs_dir));
  control_event_hs_descriptor_requested(
      rend_data->onion_address, rend_data->auth_type, hs_dir->identity_digest,
      desc_id_base32, NULL);
  return 1;
}
...
fetch_v2_desc_by_addr(rend_data_t *rend_query, smartlist_t *hsdirs)
{
  ...
  while (tries_left > 0) {
    ...
    ret = rend_compute_v2_desc_id(descriptor_id, rend_data->onion_address,
...
rend_client_fetch_v2_desc(rend_data_t *query, smartlist_t *hsdirs)
{
  ...
  if (onion_address[0] != '\0') {
    ...
    ret = fetch_v2_desc_by_descid(rend_data->desc_id_fetch, query, hsdirs);
  } else {
      ...
      switch (failure_type) {
        ...
        case INTRO_POINT_FAILURE_GENERIC:
          rend_cache_intro_failure_note(
              failure_type, (uint8_t *)failed_intro->identity_digest,
              onion_address);
        ...
        case INTRO_POINT_FAILURE_UNREACHABLE:
          ...
          {
            int zap_intro_point = intro->unreachable_count >=
...
rend_client_desc_trynow(const char *query)
{
  ...
  SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
    ...
    if (rend_cache_lookup_entry(onion_address, -1, &entry) == 0 &&
...
rend_client_note_connection_attempt_ended(const rend_data_t *rend_data)
{
  ...
  if (cache_entry != NULL) {
    SMARTLIST_FOREACH(cache_entry->parsed->intro_nodes, rend_intro_point_t *,
                      ip, ip->timed_out = 0;);
  }
                                  ...
                                  const int strict, const int warnings)
{
  ...
  SMARTLIST_FOREACH_BEGIN (usable_nodes, const rend_intro_point_t *, ip) {
    bool failed_intro = rend_cache_intro_failure_exists(
        service_id, (const uint8_t *)ip->extend_info->identity_digest);
...
again:
  ...
  if (strict && routerset_contains_extendinfo(options->ExcludeNodes,
                                              intro->extend_info)) {
    n_excluded++;
    smartlist_del(usable_nodes, i);
    goto again;
                                 ...
                                 int validate_only)
{
  ...
  for (line = options->HidServAuth; line; line = line->next) {
    ...
    strlcpy(auth->onion_address, onion_address,
...
rend_client_circuit_cleanup_on_free(const circuit_t *circ)
{
  ...
  switch (circ->purpose) {
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT: {
      ...
      log_info(
          LD_REND, "Failed intro circ %s to %s (awaiting ack). %s",
          safe_str_client(rend_data_get_address(ocirc->rend_data)),
          safe_str_client(build_state_get_exit_nickname(ocirc->build_state)),
          has_timed_out ? "Recording timeout." : "Removing from descriptor.");
      rend_client_report_intro_point_failure(
          ocirc->build_state->chosen_exit, ocirc->rend_data,
          has_timed_out ? INTRO_POINT_FAILURE_TIMEOUT
    ...
    case CIRCUIT_PURPOSE_C_INTRODUCING: {
      ...
      if (has_timed_out || ! ocirc->build_state->chosen_exit ||
          ip_is_redundant) {
        break;
      }
      log_info(
