...
find_expiring_intro_point(struct rend_service_t *service,
...
static int rend_service_check_private_dir(const or_options_t *options,
                                          const rend_service_t *s, int create);
...
rend_add_service(smartlist_t *service_list, rend_service_t *service)
{
  ...
  for (i = 0; i < smartlist_len(service->ports); ++i) {
    ...
    if (! (p->is_unix_addr)) {
      log_debug(LD_REND, "Service maps port %d to %s", p->virtual_port,
                fmt_addrport(&p->real_addr, p->real_port));
    } else {
#ifdef HAVE_SYS_UN_H
      log_debug(LD_REND, "Service maps port %d to socket at \"%s\"",
                               ...
                               char **err_msg_out)
{
  ...
  virtport =
      (int)tor_parse_long(smartlist_get(sl, 0), 10, 1, 65535, NULL, NULL);
  if (! virtport) {
  ...
  if (smartlist_len(sl) == 1) {
    ...
    int is_unix;
    ret = port_cfg_line_extract_addrport(addrport_element, &addrport, &is_unix,
                                         &rest);
...
rend_service_prune_list_impl_(void)
{
  ...
  while ((ocirc = circuit_get_next_intro_circ(ocirc, false))) {
    ...
    log_info(
        LD_REND, "Closing intro point %s for service %s.",
        safe_str_client(extend_info_describe(ocirc->build_state->chosen_exit)),
...
int
rend_config_service(const config_line_t *line_, const or_options_t *options,
                    hs_service_config_t *config)
{
  ...
  for (line = line_; line; line = line->next) {
    ...
    if (! strcasecmp(line->key, "HiddenServiceNumIntroductionPoints")) {
      ...
      service->n_intro_points_wanted = (unsigned int)tor_parse_long(
          line->value, 10, 0, NUM_INTRO_POINTS_MAX, &ok, NULL);
      if (! ok) {
        log_warn(LD_CONFIG,
...
rend_service_add_ephemeral(crypto_pk_t *pk, smartlist_t *ports,
...
rend_service_del_ephemeral(const char *service_id)
{
  ...
  SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
    if (! circ->marked_for_close &&
      ...
      log_debug(
          LD_REND, "Closing intro point %s for service %s.",
          safe_str_client(extend_info_describe(oc->build_state->chosen_exit)),
  ...
  tor_assert_nonfatal(intro_period_elapsed >= 0);
  {
    ...
    if ((msg = rate_limit_log(&rlimit, approx_time()))) {
      log_fn(severity, LD_REND,
             ...
             service->n_intro_circuits_launched, (int)intro_period_elapsed,
             rend_max_intro_circs_per_period(service->n_intro_points_wanted),
...
rend_service_update_descriptor(rend_service_t *service)
{
  ...
  if (have_intro != service->n_intro_points_wanted) {
    ...
    log_fn(severity, LD_REND,
           ...
           service->service_id, service->n_intro_points_wanted, have_intro);
                                         ...
                                         const or_options_t *options)
{
  ...
  if (! rend_service_private_key_exists(s)) {
    if (poison_new_single_onion_hidden_service_dir_impl(s, options) < 0) {
      return -1;
    }
  }
...
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
  ...
  if (! (cfile = start_writing_to_stdio_file(
             cfname, OPEN_FLAGS_REPLACE | O_TEXT, 0600, &open_cfile))) {
    log_warn(LD_CONFIG, "Could not open client_keys file %s", escaped(cfname));
    goto err;
  }

  if (! (hfile = start_writing_to_stdio_file(
             hfname, OPEN_FLAGS_REPLACE | O_TEXT, 0600, &open_hfile))) {
  ...
  SMARTLIST_FOREACH_BEGIN (s->clients, rend_authorized_client_t *, client) {
    ...
    if (base64_encode(desc_cook_out, 3 * REND_DESC_COOKIE_LEN_BASE64 + 1,
                      (char *)client->descriptor_cookie, REND_DESC_COOKIE_LEN,
                      0) < 0) {
     ...
     * client_keys. */
    char *encoded_cookie =
        rend_auth_encode_cookie(client->descriptor_cookie, s->auth_type);
    if (! encoded_cookie) {
      log_warn(LD_BUG, "Could not base64-encode descriptor cookie.");
      goto err;
    }
    tor_snprintf(buf, sizeof(buf), "%s.onion %s # client: %s\n", service_id,
                 encoded_cookie, client->client_name);
...
rend_service_get_by_pk_digest(const char *digest)
{
  SMARTLIST_FOREACH(rend_service_list, rend_service_t *, s,
                    if (tor_memeq(s->pk_digest, digest, DIGEST_LEN)) return s);
  return NULL;
}
...
rend_check_authorization(rend_service_t *service,
                         const char *descriptor_cookie, size_t cookie_len)
{
                                  ...
                                  const uint8_t *request, size_t request_len)
{
  ...
  char
      keys[DIGEST_LEN + CPATH_KEY_MATERIAL_LEN]; /* Holds KH, Df, Db, Kf, Kb */
  int i;
  crypto_dh_t *dh = NULL;
  ...
  base32_encode(serviceid, REND_SERVICE_ID_LEN_BASE32 + 1, rend_pk_digest,
                REND_SERVICE_ID_LEN);
  ...
  replay = replaycache_add_test_and_elapsed(
      intro_point->accepted_intro_rsa_parts, parsed_req->ciphertext,
      MIN(parsed_req->ciphertext_len, keylen), &elapsed);
   ...
   * part 1. */
  replay = replaycache_add_test_and_elapsed(service->accepted_intro_dh_parts,
  ...
  if (crypto_dh_compute_secret(LOG_PROTOCOL_WARN, dh, (char *)(parsed_req->dh),
                               DH1024_KEY_LEN, keys,
  ...
  for (i = 0; i < max_rend_failures; i++) {
    ...
    launched = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND,
                                             rp, flags);
  ...
  if (! launched) { /* give up */
    log_warn(LD_REND,
             ...
             safe_str_client(extend_info_describe(rp)), serviceid);
    reason = END_CIRC_REASON_CONNECTFAILED;
    goto err;
  }
  log_info(LD_REND,
           ...
           safe_str_client(extend_info_describe(rp)), hexcookie, serviceid);
  tor_assert(launched->build_state);
  /* Fill in the circuit's state. */

  launched->rend_data = rend_data_service_create(
  ...
  dh = NULL;
  if (cpath_init_circuit_crypto(cpath, keys + DIGEST_LEN,
                                sizeof(keys) - DIGEST_LEN, 1, 0) < 0)
    goto err;
...
log_error:
  if (! err_msg) {
    if (stage_descr) {
      tor_asprintf(&err_msg, "unknown %s error for INTRODUCE2", stage_descr);
    } else {
...
find_rp_for_intro(const rend_intro_cell_t *intro, char **err_msg_out)
{
  extend_info_t *rp = NULL;
  char *err_msg = NULL;
  ...
  if (intro->version == 0 || intro->version == 1) {
    ...
    if (err_msg_out) {
      tor_asprintf(&err_msg, "Unknown version %d in INTRODUCE2 cell",
                   (int)(intro->version));
    }
...
rend_intro_cell_t *
rend_service_begin_parse_intro(const uint8_t *request, size_t request_len,
                               uint8_t type, char **err_msg_out)
{
  ...
  if (request_len < (DIGEST_LEN + REND_COOKIE_LEN + (MAX_NICKNAME_LEN + 1) +
                     DH1024_KEY_LEN + 42)) {
    if (err_msg_out) {
      tor_asprintf(&err_msg, "got a truncated INTRODUCE%d cell", (int)type);
    }
    goto err;
  }
...
err:
  ...
  if (err_msg_out && ! err_msg) {
    tor_asprintf(&err_msg, "unknown INTRODUCE%d error", (int)type);
  }
...
static ssize_t
rend_service_parse_intro_for_v0_or_v1(rend_intro_cell_t *intro,
                                      const uint8_t *buf, size_t plaintext_len,
                                      char **err_msg_out)
{
  ...
  if ((intro->version == 0 && ! is_legal_nickname(rp_nickname)) ||
      (intro->version == 1 && ! is_legal_nickname_or_hexdigest(rp_nickname))) {
    if (err_msg_out) {
      tor_asprintf(err_msg_out, "bad nickname in INTRODUCE%d cell",
                   (int)(intro->type));
    }
    goto err;
  }
...
static ssize_t
rend_service_parse_intro_for_v2(rend_intro_cell_t *intro, const uint8_t *buf,
                                size_t plaintext_len, char **err_msg_out)
{
  unsigned int klen;
   ...
   */
  if (! (intro->version == 2 || intro->version == 3)) {
    if (err_msg_out)
      tor_asprintf(err_msg_out,
  ...
  if (intro->version == 2)
    intro->u.v2.extend_info = extend_info;
  else
    intro->u.v3.extend_info = extend_info;
...
static ssize_t
rend_service_parse_intro_for_v3(rend_intro_cell_t *intro, const uint8_t *buf,
                                size_t plaintext_len, char **err_msg_out)
{
    ...
    if (intro->u.v3.auth_len != REND_DESC_COOKIE_LEN) {
      if (err_msg_out) {
        tor_asprintf(err_msg_out,
                     ...
                     "should be %d",
                     (int)(intro->u.v3.auth_len), (int)(intro->type),
                     REND_DESC_COOKIE_LEN);
      }
  ...
  if (intro->u.v3.auth_type != REND_NO_AUTH && intro->u.v3.auth_len > 0) {
...
static ssize_t (*intro_version_handlers[])(rend_intro_cell_t *,
                                           const uint8_t *, size_t,
                                           char **) = {
    rend_service_parse_intro_for_v0_or_v1,
    rend_service_parse_intro_for_v0_or_v1, rend_service_parse_intro_for_v2,
...
int
rend_service_decrypt_intro(rend_intro_cell_t *intro, crypto_pk_t *key,
                           char **err_msg_out)
{
  char *err_msg = NULL;
  ...
  if (! intro || ! key) {
    if (err_msg_out) {
      err_msg = tor_strdup("rend_service_decrypt_intro() called with bad "
                           "parameters");
    }
  ...
  if (tor_memneq(key_digest, intro->pk, DIGEST_LEN)) {
    if (err_msg_out) {
      ...
      tor_asprintf(&err_msg,
                   ...
                   (int)(intro->type), escaped(service_id));
    }
  ...
  result = crypto_pk_obsolete_private_hybrid_decrypt(
      key, (char *)buf, sizeof(buf), (const char *)(intro->ciphertext),
      intro->ciphertext_len, PK_PKCS1_OAEP_PADDING, 1);
  if (result < 0) {
    if (err_msg_out) {
      tor_asprintf(&err_msg, "couldn't decrypt INTRODUCE%d cell",
                   (int)(intro->type));
    }
    status = -6;
    goto err;
...
int
rend_service_parse_intro_plaintext(rend_intro_cell_t *intro,
                                   char **err_msg_out)
{
  char *err_msg = NULL;
  ...
  ver_specific_len = intro_version_handlers[version](
      intro, intro->plaintext, intro->plaintext_len, &err_msg);
  ...
  if (ver_invariant_len < REND_COOKIE_LEN + DH1024_KEY_LEN) {
    tor_asprintf(
        ...
        (int)(intro->type), (long)(intro->plaintext_len));
    status = -5;
    goto err;
    ...
    memcpy(intro->rc, intro->plaintext + ver_specific_len, REND_COOKIE_LEN);
    memcpy(intro->dh, intro->plaintext + ver_specific_len + REND_COOKIE_LEN,
           DH1024_KEY_LEN);
  }
...
err:
  if (err_msg_out && ! err_msg) {
    tor_asprintf(&err_msg, "unknown INTRODUCE%d error parsing encrypted part",
                 intro ? (int)(intro->type) : -1);
  }
                                 ...
                                 char **err_msg_out)
{
  ...
  if (! intro) {
    if (err_msg_out)
      *err_msg_out = tor_strdup("NULL intro cell passed to "
  ...
  if (intro->version == 3 && intro->parsed) {
    if (! (intro->u.v3.auth_type == REND_NO_AUTH ||
      ...
      if (err_msg_out)
        tor_asprintf(err_msg_out, "unknown authorization type %d",
                                    ...
                                    rend_intro_point_t *intro)
{
  ...
  tor_assert(tor_memeq(intro->extend_info->identity_digest,
                       launch_ei->identity_digest, DIGEST_LEN));
  ...
  if (! launched) {
    log_info(
        LD_REND, "Can't launch circuit to establish introduction at %s%s%s.",
        safe_str_client(extend_info_describe(intro->extend_info)),
        direct_ei ? " via direct address " : "",
        direct_ei ? safe_str_client(extend_info_describe(direct_ei)) : "");
  ...
  launched->rend_data = rend_data_service_create(
      service->service_id, service->pk_digest, NULL, service->auth_type);
                                         ...
                                         const char *rend_circ_nonce)
{
  ...
  r = crypto_pk_private_sign_digest(intro_key, cell_body_out + len,
                                    cell_body_out_len - len, cell_body_out,
                                    len);
  if (r < 0) {
...
rend_service_intro_has_opened(origin_circuit_t *circuit)
{
  ...
  {
    ssize_t len;
    len = rend_service_encode_establish_intro_cell(
        buf, sizeof(buf), circuit->intro_key,
    ...
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circuit),
                                     RELAY_COMMAND_ESTABLISH_INTRO, buf, len,
...
rend_service_rendezvous_has_opened(origin_circuit_t *circuit)
{
  ...
  if (crypto_dh_get_public(hop->rend_dh_handshake_state, buf + REND_COOKIE_LEN,
                           DH1024_KEY_LEN) < 0) {
  ...
  if (relay_send_command_from_edge(
          0, TO_CIRCUIT(circuit), RELAY_COMMAND_RENDEZVOUS1, buf,
          HS_LEGACY_RENDEZVOUS_CELL_SIZE, circuit->cpath->prev) < 0) {
...
find_intro_circuit(rend_intro_point_t *intro, const char *pk_digest)
{
  ...
  while ((circ = circuit_get_next_by_pk_and_purpose(
              circ, (uint8_t *)pk_digest, CIRCUIT_PURPOSE_S_INTRO))) {
  ...
  circ = NULL;
  while (
      (circ = circuit_get_next_by_pk_and_purpose(
           circ, (uint8_t *)pk_digest, CIRCUIT_PURPOSE_S_ESTABLISH_INTRO))) {
    if (tor_memeq(circ->build_state->chosen_exit->identity_digest,
                  intro->extend_info->identity_digest, DIGEST_LEN) &&
        circ->rend_data) {
      return circ;
...
find_expiring_intro_point(rend_service_t *service, origin_circuit_t *circ)
{
  ...
  SMARTLIST_FOREACH(
      service->expiring_nodes, rend_intro_point_t *, intro_point,
...
find_intro_point(origin_circuit_t *circ)
{
  ...
  SMARTLIST_FOREACH(
      rend_service_list, rend_service_t *, s,
  ...
  SMARTLIST_FOREACH(
      service->intro_nodes, rend_intro_point_t *, intro_point,
      if (crypto_pk_eq_keys(intro_point->intro_key, circ->intro_key)) {
        return intro_point;
      });
  ...
  for (i = 0; i < smartlist_len(descs); i++) {
    ...
    if (hs_dirs && smartlist_len(hs_dirs) > 0) {
      ...
      if (hid_serv_get_responsible_directories(responsible_dirs,
        ...
        control_event_hs_descriptor_upload(service_id, "UNKNOWN", "UNKNOWN",
                                           NULL);
        goto done;
      }
    }
    for (j = 0; j < smartlist_len(responsible_dirs); j++) {
      ...
      directory_request_set_payload(req, desc->desc_str,
      ...
      base32_encode(desc_id_base32, sizeof(desc_id_base32), desc->desc_id,
                    DIGEST_LEN);
      ...
      log_info(LD_REND,
               ...
               "%s:%d.",
               safe_str_client(service_id), safe_str_client(desc_id_base32),
               seconds_valid, hs_dir->nickname, hs_dir_ip, hs_dir->or_port);
      control_event_hs_descriptor_upload(service_id, hs_dir->identity_digest,
                                         desc_id_base32, NULL);
      tor_free(hs_dir_ip);
    ...
    for (j = 0; j < num_descs; j++) {
      ...
      switch (service->auth_type) {
        ...
        case REND_BASIC_AUTH:
          SMARTLIST_FOREACH(
              service->clients, rend_authorized_client_t *, cl,
      ...
      seconds_valid = rend_encode_v2_descriptors(descs, service->desc, now, 0,
                                                 service->auth_type,
      ...
      if (get_options()->PublishHidServDescriptors) {
        ...
        log_info(LD_REND, "Launching upload for hidden service %s", serviceid);
      ...
      if (seconds_valid - REND_TIME_PERIOD_OVERLAPPING_V2_DESCS >
        ...
        service->next_upload_time =
            now + seconds_valid - REND_TIME_PERIOD_OVERLAPPING_V2_DESCS + 1;
 ...
 * it). */
static int
intro_point_should_expire_now(rend_intro_point_t *intro, time_t now)
{
  ...
  if (intro->time_to_expire == -1) {
    ...
    int intro_point_lifetime_seconds = crypto_rand_int_range(
        INTRO_POINT_LIFETIME_MIN_SECONDS, INTRO_POINT_LIFETIME_MAX_SECONDS);
                            ...
                            smartlist_t *retry_nodes, time_t now)
{
  ...
  SMARTLIST_FOREACH_BEGIN (service->intro_nodes, rend_intro_point_t *, intro) {
    ...
    const node_t *node = node_get_by_id(intro->extend_info->identity_digest);
    ...
    for (i = 0; i < (int)n_intro_points_to_open; i++) {
      ...
      if (! node) {
        log_warn(LD_REND,
                 ...
                 safe_str_client(service->service_id), n_intro_points_to_open);
        break;
      }
...
rend_consider_services_upload(time_t now)
{
  ...
  int rendinitialpostdelay =
      (options->TestingTorNetwork ? MIN_REND_INITIAL_POST_DELAY_TESTING
...
rend_service_dump_stats(int severity)
{
  ...
  for (i = 0; i < smartlist_len(rend_service_list); ++i) {
    ...
    tor_log(severity, LD_GENERAL,
            "Service configured in %s:", rend_service_escaped_dir(service));
    for (j = 0; j < smartlist_len(service->intro_nodes); ++j) {
      ...
      if (! circ) {
        tor_log(severity, LD_GENERAL, "  Intro point %d at %s: no circuit", j,
                safe_name);
        continue;
      }
      tor_log(severity, LD_GENERAL, "  Intro point %d at %s: circuit is %s", j,
              safe_name, circuit_state_to_string(circ->base_.state));
    }
  }
}
...
#define MAX_STREAM_WARN_INTERVAL 600
    ...
    if (circ->rend_data->nr_streams >= service->max_streams_per_circuit) {
      log_fn_ratelim(
          &stream_ratelim, LOG_WARN, LD_REND,
          "Maximum streams per circuit limit reached on rendezvous "
          "circuit %u; %s.  Circuit has %d out of %d streams.",
          (unsigned)circ->base_.n_circ_id,
          service->max_streams_close_circuit ? "closing circuit"
                                             : "ignoring open stream request",
          circ->rend_data->nr_streams, service->max_streams_per_circuit);
