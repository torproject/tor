                         ...
                         const hs_service_t *service)
{
  ...
  intro_keys = get_introduce2_key_material(
      data->auth_pk, data->enc_kp, data->subcredential, encrypted_section,
      &data->client_pk);
  if (intro_keys == NULL) {
    log_info(LD_REND,
   ...
   * in the cell is at the end of the encrypted section. */
  {
    ...
    compute_introduce_mac(
        ...
        encrypted_section, encrypted_section_len, intro_keys->mac_key,
        sizeof(intro_keys->mac_key), mac, sizeof(mac));
