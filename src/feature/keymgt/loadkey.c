    ...
    case FN_EMPTY:
      if (generate) {
        ...
        if (crypto_pk_write_private_key_to_filename(prkey, fname)) {
          tor_log(severity, LD_FS, "Couldn't write generated key to \"%s\".",
                  fname);
          goto error;
        }
      } else {
...
/* DOCDOC */
static ssize_t
do_getpass(const char *prompt, char *buf, size_t buflen, int twice,
...
/* DOCDOC */
int
read_encrypted_secret_key(ed25519_secret_key_t *out, const char *fname)
{
  int r = -1;
  uint8_t *secret = NULL;
  size_t secret_len = 0;
  ...
  ssize_t encrypted_len = crypto_read_tagged_contents_from_file(
      fname, ENC_KEY_HEADER, &tag, encrypted_key, sizeof(encrypted_key));
  if (encrypted_len < 0) {
    saved_errno = errno;
  ...
  while (1) {
    ssize_t pwlen = do_getpass("Enter passphrase for master key:", pwbuf,
                               sizeof(pwbuf), 0, get_options());
    if (pwlen < 0) {
      saved_errno = EINVAL;
      goto done;
    }
    const int r_unbox = crypto_unpwbox(&secret, &secret_len, encrypted_key,
                                       encrypted_len, pwbuf, pwlen);
...
/* DOCDOC */
int
write_encrypted_secret_key(const ed25519_secret_key_t *key, const char *fname)
{
  int r = -1;
  char pwbuf0[256];
  ...
  if (crypto_pwbox(&encrypted_key, &encrypted_len, key->seckey,
                   sizeof(key->seckey), pwbuf0, strlen(pwbuf0), 0) < 0) {
    log_warn(LD_OR, "crypto_pwbox failed!?");
    goto done;
  }
  if (crypto_write_tagged_contents_to_file(fname, ENC_KEY_HEADER, ENC_KEY_TAG,
...
write_secret_key(const ed25519_secret_key_t *key, int encrypted,
                 const char *fname, const char *fname_tag,
 ...
 */
ed25519_keypair_t *
ed_key_init_from_file(const char *fname, uint32_t flags, int severity,
                      const ed25519_keypair_t *signing_key, time_t now,
                      time_t lifetime, uint8_t cert_type,
                      ...
                      const or_options_t *options)
{
  ...
  int have_secret = 0;
  int load_secret = try_to_load && ! offline_secret &&
                    (! omit_secret || file_status(public_fname) == FN_NOENT);
  if (load_secret) {
    int rv = ed25519_seckey_read_from_file(&keypair->seckey, &got_tag,
                                           secret_fname);
    if (rv == 0) {
      have_secret = 1;
  ...
  if (! have_secret && try_to_load && encrypt_key) {
    int r =
        read_encrypted_secret_key(&keypair->seckey, encrypted_secret_fname);
    if (r > 0) {
      have_secret = 1;
  ...
  if (try_to_load && (! have_secret || split)) {
    ...
    tor_free(got_tag);
    found_public = ed25519_pubkey_read_from_file(&pubkey_tmp, &got_tag,
                                                 public_fname) == 0;
      ...
      if (have_secret) {
        ...
        if (! ed25519_pubkey_eq(&keypair->pubkey, &pubkey_tmp)) {
          tor_log(severity, LD_OR,
                  ...
                  public_fname, loaded_secret_fname, loaded_secret_fname,
                  public_fname);
          goto err;
        }
      } else {
      ...
      if (have_secret) {
        if (ed25519_pubkey_write_to_file(&keypair->pubkey, public_fname, tag) <
          ...
          tor_log(LOG_NOTICE, LD_OR,
                  "Found secret key but not %s. Regenerating.", public_fname);
        }
      }
    }
  }
    ...
    if (have_encrypted_secret_file) {
      ...
      tor_log(
          ...
          (flags & INIT_ED_KEY_SUGGEST_KEYGEN)
              ...
              : "Did you forget to copy it over when you copied the rest of "
                "the "
  ...
  if (! have_secret && ! found_public && ! (flags & INIT_ED_KEY_CREATE)) {
    if (split) {
      tor_log(severity, LD_OR, "No key found in %s or %s.", secret_fname,
              public_fname);
    } else {
  ...
  if (! have_secret && ! found_public) {
    tor_free(keypair);
    keypair = ed_key_new(signing_key, flags, now, lifetime, cert_type, &cert);
    if (! keypair) {
   ...
   * we just created it. */
  if (created_sk ||
      (have_secret && options != NULL && options->change_key_passphrase)) {
    if (write_secret_key(&keypair->seckey, encrypt_key, secret_fname, tag,
                         encrypted_secret_fname) < 0 ||
        (split && ed25519_pubkey_write_to_file(&keypair->pubkey, public_fname,
                                               tag) < 0) ||
        (cert && crypto_write_tagged_contents_to_file(
                     cert_fname, "ed25519v1-cert", tag, cert->encoded,
                     cert->encoded_len) < 0)) {
  ...
  ssize_t cert_body_len = crypto_read_tagged_contents_from_file(
      cert_fname, "ed25519v1-cert", &got_tag, certbuf, sizeof(certbuf));
  ...
  cert = tor_cert_create(signing_key, cert_type, &keypair->pubkey, now,
                         lifetime, cert_flags);
  ...
  /* Write it to disk. */
  created_cert = 1;
  if (crypto_write_tagged_contents_to_file(cert_fname, "ed25519v1-cert", tag,
                                           cert->encoded,
                                           cert->encoded_len) < 0) {
 ...
 */
ed25519_keypair_t *
ed_key_new(const ed25519_keypair_t *signing_key, uint32_t flags, time_t now,
           time_t lifetime, uint8_t cert_type, struct tor_cert_st **cert_out)
{
  if (cert_out)
    *cert_out = NULL;
  ...
  if (flags & INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT)
    cert_flags |= CERT_FLAG_INCLUDE_SIGNING_KEY;
  tor_cert_t *cert = tor_cert_create(signing_key, cert_type, &keypair->pubkey,
                                     now, lifetime, cert_flags);
  if (! cert)
    goto err;
