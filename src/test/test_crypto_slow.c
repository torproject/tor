...
static void
run_s2k_tests(const unsigned flags, const unsigned type, int speclen,
              const int keylen, int legacy)
{
  ...
  r = secret_to_key_new(buf, sizeof(buf), &sz, pw1, strlen(pw1), flags);
  ...
  tt_int_op(S2K_OKAY, OP_EQ, secret_to_key_check(buf, sz, pw1, strlen(pw1)));
  ...
  tt_int_op(S2K_OKAY, OP_EQ,
            secret_to_key_derivekey(buf2, sizeof(buf2), buf, speclen, pw1,
                                    strlen(pw1)));
  ...
  memset(buf3, 0, sizeof(buf3));
  tt_int_op(S2K_OKAY, OP_EQ,
            secret_to_key_derivekey(buf3, sizeof(buf3), buf, speclen, pw1,
                                    strlen(pw1)));
...
test_libscrypt_eq_openssl(void *arg)
{
  ...
  libscrypt_retval = libscrypt_scrypt(
      (const uint8_t *)"", 0, (const uint8_t *)"", 0, N, r, p, buf1, dk_len);
  openssl_retval =
      EVP_PBE_scrypt((const char *)"", 0, (const unsigned char *)"", 0, N, r,
                     p, maxmem, buf2, dk_len);
  ...
  libscrypt_retval = libscrypt_scrypt(
      (const uint8_t *)"password", strlen("password"), (const uint8_t *)"NaCl",
      strlen("NaCl"), N, r, p, buf1, dk_len);
  openssl_retval =
      EVP_PBE_scrypt((const char *)"password", strlen("password"),
                     (const unsigned char *)"NaCl", strlen("NaCl"), N, r, p,
                     maxmem, buf2, dk_len);
  ...
  libscrypt_retval = libscrypt_scrypt(
      (const uint8_t *)"pleaseletmein", strlen("pleaseletmein"),
      (const uint8_t *)"SodiumChloride", strlen("SodiumChloride"), N, r, p,
      buf1, dk_len);
  openssl_retval =
      EVP_PBE_scrypt((const char *)"pleaseletmein", strlen("pleaseletmein"),
                     (const unsigned char *)"SodiumChloride",
                     strlen("SodiumChloride"), N, r, p, maxmem, buf2, dk_len);
...
test_crypto_s2k_errors(void *arg)
{
  ...
  tt_int_op(S2K_BAD_LEN, OP_EQ,
            secret_to_key_derivekey(buf, sizeof(buf), (const uint8_t *)"", 0,
                                    "ABC", 3));
  tt_int_op(S2K_BAD_ALGORITHM, OP_EQ,
            secret_to_key_derivekey(buf, sizeof(buf), (const uint8_t *)"\x10",
                                    1, "ABC", 3));
  tt_int_op(S2K_BAD_LEN, OP_EQ,
  ...
  tt_int_op(
      S2K_BAD_LEN, OP_EQ,
      secret_to_key_derivekey(buf2, sizeof(buf2), buf, sizeof(buf), "ABC", 3));

  /* Truncated output */
#ifdef HAVE_LIBSCRYPT
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_new(buf, 50, &sz, "ABC", 3, 0));
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_new(buf, 50, &sz, "ABC", 3, S2K_FLAG_LOW_MEM));
#endif /* defined(HAVE_LIBSCRYPT) */
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_new(buf, 37, &sz, "ABC", 3, S2K_FLAG_USE_PBKDF2));
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_new(buf, 29, &sz, "ABC", 3, S2K_FLAG_NO_SCRYPT));

#ifdef HAVE_LIBSCRYPT
  tt_int_op(S2K_TRUNCATED, OP_EQ, secret_to_key_make_specifier(buf, 18, 0));
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_make_specifier(buf, 18, S2K_FLAG_LOW_MEM));
#endif
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_make_specifier(buf, 17, S2K_FLAG_USE_PBKDF2));
  tt_int_op(S2K_TRUNCATED, OP_EQ,
            secret_to_key_make_specifier(buf, 9, S2K_FLAG_NO_SCRYPT));
  ...
  tt_int_op(S2K_BAD_PARAMS, OP_EQ,
            secret_to_key_derivekey(buf2, sizeof(buf2), buf, 18, "ABC", 3));
  ...
  tt_int_op(S2K_BAD_PARAMS, OP_EQ,
            secret_to_key_derivekey(buf2, sizeof(buf2), buf, 19, "ABC", 3));
#endif /* defined(HAVE_LIBSCRYPT) */
...
test_crypto_scrypt_vectors(void *arg)
{
  ...
  */
  ...
  base16_decode((char *)spec, sizeof(spec), "0400", 4);
  memset(out, 0x00, sizeof(out));
  tt_int_op(64, OP_EQ, secret_to_key_compute_key(out, 64, spec, 2, "", 0, 2));
  test_memeq_hex(out, "77d6576238657b203b19ca42c18a0497"
  ...
  test_memeq_hex(out, "fdbabe1c9d3472007856e7190d01e9fe"
  ...
  tt_int_op(
      64, OP_EQ,
      secret_to_key_compute_key(out, 64, spec, 16, "pleaseletmein", 13, 2));
  test_memeq_hex(out, "7023bdcb3afd7348461c06cd81fd38eb"
  ...
  tt_int_op(
      64, OP_EQ,
      secret_to_key_compute_key(out, 64, spec, 16, "pleaseletmein", 13, 2));
  test_memeq_hex(out, "2101cb9b6a511aaeaddbbe09cf70f881"
...
test_crypto_pbkdf2_vectors(void *arg)
{
  ...
  */
  ...
  base16_decode((char *)spec, sizeof(spec),
                ...
                "3414c5473616c7453414c5473616c74"
                "0C",
                74);
  memset(out, 0x00, sizeof(out));
  tt_int_op(25, OP_EQ,
...
test_crypto_pwbox(void *arg)
{
  ...
  const unsigned flags[] = {0, S2K_FLAG_NO_SCRYPT, S2K_FLAG_LOW_MEM,
  ...
  for (i = 0; i < ARRAY_LENGTH(flags); ++i) {
    tt_int_op(0, OP_EQ,
              crypto_pwbox(&boxed, &len, (const uint8_t *)msg, strlen(msg), pw,
                           strlen(pw), flags[i]));
    tt_assert(boxed);
    tt_assert(len > 128 + 32);

    tt_int_op(0, OP_EQ,
              crypto_unpwbox(&decoded, &dlen, boxed, len, pw, strlen(pw)));
    ...
    tt_int_op(UNPWBOX_BAD_SECRET, OP_EQ,
              crypto_unpwbox(&decoded, &dlen, boxed, len, pw, strlen(pw) - 1));
    boxed[len - 1] ^= 1;
    tt_int_op(UNPWBOX_BAD_SECRET, OP_EQ,
              crypto_unpwbox(&decoded, &dlen, boxed, len, pw, strlen(pw)));
    boxed[0] = 255;
    tt_int_op(UNPWBOX_CORRUPTED, OP_EQ,
              crypto_unpwbox(&decoded, &dlen, boxed, len, pw, strlen(pw)));
...
test_crypto_ed25519_fuzz_donna(void *arg)
{
  ...
  for (i = 0; i < iters; ++i) {
    ...
    tt_int_op(
        0, OP_EQ,
        ed25519_keypair_from_curve25519_keypair(&kp_curve25519, &bit, &ckp));
    ...
    tt_int_op(0, OP_EQ,
              ed25519_public_key_from_curve25519_public_key(&pk_curve25519,
                                                            &ckp.pubkey, bit));
