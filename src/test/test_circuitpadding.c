...
/* These machines obfuscate the circuit length by padding it to 7 hops */
static void
helper_create_circsetup_origin_machine(void)
{
  ...
  /* Start, gap, burst */
  circpad_machine_states_init(circ_origin_machine, 3);
  ...
  circ_origin_machine->states[CIRCPAD_STATE_BURST].token_removal =
      CIRCPAD_TOKEN_REMOVAL_CLOSEST;

  circ_origin_machine->states[CIRCPAD_STATE_BURST].histogram_len = 2;
  circ_origin_machine->states[CIRCPAD_STATE_BURST].histogram_edges[0]= 500;
  circ_origin_machine->states[CIRCPAD_STATE_BURST].histogram_edges[1]= 1000000;
  ...
  circ_origin_machine->states[CIRCPAD_STATE_BURST].histogram[0] = 5;

  circ_origin_machine->states[CIRCPAD_STATE_BURST].histogram_total_tokens = 5;
...
helper_create_circsetup_relay_machine(void)
{
  ...
  /* Start, gap, burst */
  circpad_machine_states_init(circ_relay_machine, 3);
  ...
  circ_relay_machine->states[CIRCPAD_STATE_BURST].histogram[0] = 0;
  // Only infinity bin:
  circ_relay_machine->states[CIRCPAD_STATE_BURST].histogram[1] = 1;
  circ_relay_machine->states[CIRCPAD_STATE_BURST].
      histogram_total_tokens = 1;

  /* From the gap state, we _stay_ in the gap state, when we receive padding
   * or non padding */
  circ_relay_machine->states[CIRCPAD_STATE_GAP].
      next_state[CIRCPAD_EVENT_PADDING_RECV] = CIRCPAD_STATE_GAP;
  circ_relay_machine->states[CIRCPAD_STATE_GAP].
  ...
  circ_relay_machine->states[CIRCPAD_STATE_GAP].histogram[1] = 1;
  circ_relay_machine->states[CIRCPAD_STATE_GAP].histogram[2] = 2;
  circ_relay_machine->states[CIRCPAD_STATE_GAP].histogram[3] = 2;
  circ_relay_machine->states[CIRCPAD_STATE_GAP].histogram[4] = 1;
  /* Total number of tokens */
  circ_relay_machine->states[CIRCPAD_STATE_GAP].histogram_total_tokens = 6;

  circ_relay_machine->states[CIRCPAD_STATE_GAP].token_removal =
