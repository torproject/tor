...
// msg.h macros and just test the dispatch core.  We'll use a string type
// for everything.
...
#define ADD_PUBLISH(binding_suffix, subsys, channel, msg, flags)        \
  STMT_BEGIN {                                                          \
    con = dispatch_connector_for_subsystem(builder,                     \
                                           get_subsys_id(#subsys));     \
    dispatch_add_pub_(con, &pub_binding_##binding_suffix,               \
                      get_channel_id(#channel),                         \
                      get_message_id(#msg), get_msg_type_id("string"),  \
                      (flags), __FILE__, __LINE__);                     \
    dispatch_connector_free(con);                                       \
  } STMT_END
...
#define ADD_SUBSCRIBE(hook_suffix, subsys, channel, msg, flags)         \
  STMT_BEGIN {                                                          \
    con = dispatch_connector_for_subsystem(builder,                     \
                                           get_subsys_id(#subsys));     \
    dispatch_add_sub_(con, recv_msg_##hook_suffix,                      \
                      get_channel_id(#channel),                         \
                      get_message_id(#msg), get_msg_type_id("string"),  \
                      (flags), __FILE__, __LINE__);                     \
    dispatch_connector_free(con);                                       \
...
#define SEND(binding_suffix, val)                          \
  STMT_BEGIN {                                             \
    ...
    dispatch_pub_(&pub_binding_##binding_suffix, data_);   \
  } STMT_END
...
{
  (void)testcase;
  dispatch_builder_t *builder = dispatch_builder_new();
  dispatch_connector_t *con;

  {
    con = dispatch_connector_for_subsystem(builder, get_subsys_id("types"));
  ...
  for (int i = 0; i < 1000-5; ++i) {
    ...
    con = dispatch_connector_for_subsystem(builder, get_subsys_id(sys));
    dispatch_add_sub_(con, recv_msg_copy_string,
                      get_channel_id("main"),
  ...
  return dispatch_builder_finalize(builder);
}
...
{
  (void)testcase;
  dispatcher_t *dispatcher = dispatcher_;
...
static void
test_dispatch_msg_minimal(void *arg)
{
  dispatcher_t *d = arg;
...
static void
test_dispatch_msg_send_to_stub(void *arg)
{
  dispatcher_t *d = arg;
...
static void
test_dispatch_msg_cancel_msgs(void *arg)
{
  dispatcher_t *d = arg;
...
static void
alertfn_generic(dispatcher_t *d, channel_id_t ch, void *arg)
{
...
static void
test_dispatch_msg_alertfns(void *arg)
{
  dispatcher_t *d = arg;
  ...
  tt_int_op(0, OP_EQ,
            dispatcher_set_alert_fn(d, get_channel_id("main"),
                                    alertfn_generic, &ch1_a));
  tt_int_op(0, OP_EQ,
            dispatcher_set_alert_fn(d, get_channel_id("other"),
...
static void
test_dispatch_msg_many_hooks(void *arg)
{
  dispatcher_t *d = arg;
...
#define T(name)                                                 \
  { #name, test_dispatch_msg_ ## name , TT_FORK,                \
      &dispatcher_setup, NULL }

struct testcase_t dispatch_msg_tests[] = {
  T(minimal),
  T(send_to_stub),
  T(cancel_msgs),
  T(alertfns),
