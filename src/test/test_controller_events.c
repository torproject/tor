...
add_testing_cell_stats_entry(circuit_t *circ, uint8_t command,
                             unsigned int waiting_time, unsigned int removed,
...
test_cntev_append_cell_stats(void *arg)
{
  ...
  memset(include_if_non_zero, 0, (CELL_COMMAND_MAX_ + 1) * sizeof(uint64_t));
  memset(number_to_include, 0, (CELL_COMMAND_MAX_ + 1) * sizeof(uint64_t));
...
test_cntev_format_cell_stats(void *arg)
{
  ...
  format_cell_stats(&event_string, TO_CIRCUIT(ocirc), cell_stats);
  tt_str_op("ID=2 OutboundQueue=3 OutboundConn=1 OutboundAdded=relay:4", OP_EQ,
            event_string);
  tor_free(event_string);
...
test_cntev_event_mask(void *arg)
{
  ...
  for (test_event = 0; test_event < EVENT_MIN_; test_event++)
    tt_assert(! control_event_is_interesting(test_event));
  for (test_event = EVENT_MAX_ + 1; test_event < EVENT_CAPACITY_; test_event++)
    tt_assert(! control_event_is_interesting(test_event));
  ...
  for (selected_event = EVENT_MIN_; selected_event <= EVENT_MAX_;
       selected_event++) {
    ...
    for (test_event = 0; test_event < EVENT_MIN_; test_event++)
      tt_assert(! control_event_is_interesting(test_event));
    for (test_event = EVENT_MAX_ + 1; test_event < EVENT_CAPACITY_;
         test_event++)
      tt_assert(! control_event_is_interesting(test_event));
  }
