...
test_parsecommon_tokenize_string_null(void *arg)
{
  ...
  int retval =
      tokenize_string(area, str_with_null, str_with_null + 3, tokens, NULL, 0);
...
test_parsecommon_tokenize_string_multiple_lines(void *arg)
{
  ...
  int retval = tokenize_string(area, str, NULL, tokens, table, 0);
...
test_parsecommon_tokenize_string_at_end(void *arg)
{
  ...
  char *str = tor_strdup("client-name Alice\nuptime 1024\n");
...
test_parsecommon_get_next_token_object(void *arg)
{
  ...
  tt_str_op(token->args[0], OP_EQ, "0232AF901C31A04EE9848595AF9BB7620D4C5B2E");
  tt_str_op(token->args[1], OP_EQ, "CD1FD971855430880D3C31E0331C5C55800C2F79");
