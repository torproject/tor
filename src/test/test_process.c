...
test_default_values(void *arg)
{
  ...
  process_t *process = process_new(PROCESS_TYPE_UNKNOWN);

  /* Check the process type. */
  tt_int_op(PROCESS_TYPE_UNKNOWN, OP_EQ, process_get_type(process));
...
test_stringified_types(void *arg)
{
  ...
  /* process_type_t values. */
  tt_str_op("Unknown Process", OP_EQ,
            process_type_to_string(PROCESS_TYPE_UNKNOWN));
...
test_line_protocol_simple(void *arg)
{
  ...
  /* We forgot to set the callbacks, our data should still be there though. */
