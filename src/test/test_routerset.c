...
static int NS(geoip_is_loaded)(sa_family_t family)
{
  (void)family;
...
static int NS(geoip_get_n_countries)(void)
{
...
addr_policy_t *NS(router_parse_addr_policy_item_from_string)(
    const char *s, int assume_action, int *malformed_list)
{
  (void)s;
  (void)assume_action;
...
addr_policy_t *NS(router_parse_addr_policy_item_from_string)(const char *s,
                                                             int assume_action,
                                                             int *bogus)
{
  (void)s;
...
static smartlist_t *NS(smartlist_new)(void)
{
...
country_t NS(geoip_get_country)(const char *country)
{
  (void)country;
  CALLED(geoip_get_country)++;
...
int NS(geoip_is_loaded)(sa_family_t family)
{
  CALLED(geoip_is_loaded)++;
...
const node_t *NS(node_get_by_nickname)(const char *nickname, unsigned flags)
{
...
const smartlist_t *NS(nodelist_get_list)(void)
{
...
void NS(smartlist_free_)(smartlist_t *s)
{
  (void)s;
...
void NS(strmap_free_)(strmap_t *map, void (*free_val)(void *))
{
  CALLED(strmap_free_)++;
  strmap_free___real(map, free_val);
}

void NS(digestmap_free_)(digestmap_t *map, void (*free_val)(void *))
{
