...
test_container_smartlist_strings(void *arg)
{
  ...
  /* Test bsearch. */
  smartlist_sort(sl, compare_strs_);
  tt_str_op("nickm", OP_EQ,
            smartlist_bsearch(sl, "zNicKM", cmp_without_first_));
  ...
  /* Test bsearch_idx */
  {
    ...
    tt_int_op(0, OP_EQ,
              smartlist_bsearch_idx(sl, " aaa", cmp_without_first_, &f));
    tt_int_op(f, OP_EQ, 0);
    tt_int_op(0, OP_EQ,
              smartlist_bsearch_idx(sl, " and", cmp_without_first_, &f));
    tt_int_op(f, OP_EQ, 1);
    tt_int_op(1, OP_EQ,
              smartlist_bsearch_idx(sl, " arm", cmp_without_first_, &f));
    tt_int_op(f, OP_EQ, 0);
    tt_int_op(1, OP_EQ,
    ...
    tt_int_op(
        0, OP_EQ,
        smartlist_bsearch_idx(tmp, "foo", compare_strs_for_bsearch_, &f));
    tt_int_op(f, OP_EQ, 0);
    ...
    tt_int_op(
        0, OP_EQ,
        smartlist_bsearch_idx(tmp, "aaa", compare_strs_for_bsearch_, &f));
    tt_int_op(f, OP_EQ, 0);
    tt_int_op(
        0, OP_EQ,
        smartlist_bsearch_idx(tmp, "bar", compare_strs_for_bsearch_, &f));
    tt_int_op(f, OP_EQ, 1);
    ...
    tt_int_op(
        2, OP_EQ,
        smartlist_bsearch_idx(tmp, "goo", compare_strs_for_bsearch_, &f));
    tt_int_op(f, OP_EQ, 0);
    smartlist_free(tmp);
  }
  ...
  smartlist_split_string(
      sl, "50,noon,radar,a,man,a,plan,a,canal,panama,radar,noon,50", ",", 0,
      0);
  smartlist_sort(sl, compare_strs_);
  ...
  smartlist_split_string(
      sl, "Some say the Earth will end in ice and some in fire", " ", 0, 0);
...
test_container_smartlist_digests(void *arg)
{
  ...
  tt_int_op(0, OP_EQ,
            smartlist_contains_digest(sl, "\00090AAB2AAABaasdAAAAA"));

  /* sort digests */
  smartlist_sort_digests(sl);
  tt_mem_op(smartlist_get(sl, 0), OP_EQ, "\00090AAB2AAAAaasdAAAAA",
            DIGEST_LEN);
  tt_mem_op(smartlist_get(sl, 1), OP_EQ, "\00090AAB2AAAAaasdAAAAA",
            DIGEST_LEN);
...
test_container_smartlist_join(void *arg)
{
  ...
  smartlist_split_string(sl2,
                         ...
                         " ", 0, 0);
  SMARTLIST_FOREACH_JOIN(sl, char *, cp1, sl2, char *, cp2, strcmp(cp1, cp2),
...
test_container_digestset(void *arg)
{
  ...
  SMARTLIST_FOREACH(included, const char *, cp,
                    if (digestset_probably_contains(set, cp)) ok = 0);
  tt_assert(ok);
  SMARTLIST_FOREACH(included, const char *, cp, digestset_add(set, cp));
  SMARTLIST_FOREACH(included, const char *, cp,
                    if (!digestset_probably_contains(set, cp)) ok = 0);
  tt_assert(ok);
  for (i = 0; i < 1000; ++i) {
...
test_container_fp_pair_map(void *arg)
{
  ...
  tt_ptr_op(fp_pair_map_get(map, &fp2), OP_EQ, NULL);
  tt_ptr_op(fp_pair_map_get_by_digests(map, fp5.first, fp5.second), OP_EQ,
            v104);
