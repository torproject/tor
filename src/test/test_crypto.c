...
test_crypto_dh(void *arg)
{
  ...
  s1len =
      crypto_dh_compute_secret(LOG_WARN, dh1_dup, p2, DH1024_KEY_LEN, s1, 50);
...

  {
    ...
    s1len =
        crypto_dh_compute_secret(LOG_WARN, dh1, p1, DH1024_KEY_LEN, s1, 50);
    tt_int_op(-1, OP_EQ, s1len);
    ...
    const char P[] = "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD129024E08"
    ...
    memset(p1, 0xff, DH1024_KEY_LEN), /* 2^1024-1 */
        s1len = crypto_dh_compute_secret(LOG_WARN, dh1, p1, DH1024_KEY_LEN, s1,
                                         50);
    tt_int_op(-1, OP_EQ, s1len);
  }

  {
    ...
    s1len =
        crypto_dh_compute_secret(LOG_WARN, dh2, p1, DH1024_KEY_LEN, s1, 50);
...
test_crypto_openssl_version(void *arg)
{
  ...
  int a = -1, b = -1, c = -1;
  if (! strcmpstart(version, "LibreSSL") ||
      ! strcmpstart(version, "BoringSSL"))
    return;
...
test_crypto_aes128(void *arg)
{
   ...
   * script. */
  crypto_cipher_free(env1);
  env1 = crypto_cipher_new_with_iv("\x80\x00\x00\x00\x00\x00\x00\x00"
                                   "\x00\x00\x00\x00\x00\x00\x00\x00",
                                   "\x00\x00\x00\x00\x00\x00\x00\x00"
...
test_crypto_aes_ctr_testvec(void *arg)
{
  ...
  const char plaintext16[] = "6bc1bee22e409f96e93d7e117393172a"
  ...
  if (! strcmp(bitstr, "128")) {
    ...
    key16 = "603deb1015ca71be2b73aef0857d7781"
  ...
  base16_decode(plaintext, sizeof(plaintext), plaintext16,
                strlen(plaintext16));
...
#define RFC_4231_MAX_KEY_SIZE 131
  ...
  test_memeq_hex(digest, "b0344c61d8db38535ca8afceaf0bf12b"
                         "881dc200c9833da726e9376c2e32cff7");

  /* "Test Case 2" from RFC 4231 */
  memset(key, 0x0b, 20);
  crypto_hmac_sha256(digest, "Jefe", 4, "what do ya want for nothing?", 28);
  test_memeq_hex(digest, "5bdcc146bf60754e6a042426089575c7"
  ...
  test_memeq_hex(digest, "773ea91e36800e46854db8ebd09181a7"
                         "2959098b3ef8c122d9635514ced565fe");

  /* "Test case 4" from RFC 4231 */
  base16_decode(key, 25, "0102030405060708090a0b0c0d0e0f10111213141516171819",
                50);
  memset(data, 0xcd, 50);
  crypto_hmac_sha256(digest, key, 25, data, 50);
  test_memeq_hex(digest, "82558a389a443c0ea4cc819899f2083a"
                         "85f0faa3e578f8077a2e3ff46729665b");

  /* "Test case 5" from RFC 4231 */
  memset(key, 0x0c, 20);
  crypto_hmac_sha256(digest, key, 20, "Test With Truncation", 20);
  test_memeq_hex(digest, "a3b6167473100ee06e0c796c2955552b");
  ...
  crypto_hmac_sha256(digest, key, 131,
                     ...
                     54);
  test_memeq_hex(digest, "60e431591ee0b67f0d8a26aacbf5b77f"
  ...
  crypto_hmac_sha256(digest, key, 131,
                     ...
                     152);
  test_memeq_hex(digest, "9b09ffa71b942fcb27635fbcd5b0e944"
...
test_crypto_sha3(void *arg)
{
  ...
  i = crypto_digest256(data, (const char *)keccak_kat_msg8, 1,
                       DIGEST_SHA3_256);
  test_memeq_hex(data, "677035391CD3701293D385F037BA3279"
                       "6252BB7CE180B00B582DD9B20AAAD7F0");
  tt_int_op(i, OP_EQ, 0);
  i = crypto_digest512(data, (const char *)keccak_kat_msg8, 1,
                       DIGEST_SHA3_512);
  ...
  const uint8_t keccak_kat_msg1080[] = {
      ,
  };
  ...
  const uint8_t keccak_kat_msg1088[] = {
      ,
  };
  ...
  const uint8_t keccak_kat_msg1096[] = {
      ,
  };
  ...
  const uint8_t keccak_kat_msg1144[] = {
      ,
  };
  ...
  const uint8_t keccak_kat_msg1152[] = {
      ,
  };
  ...
  const uint8_t keccak_kat_msg1160[] = {
      ...
      ,
  };
...
test_crypto_mac_sha3(void *arg)
{
  ...
  crypto_mac_sha3_256(hmac_test, sizeof(hmac_test), (uint8_t *)key,
                      strlen(key), (uint8_t *)msg, strlen(msg));
...
test_crypto_pk(void *arg)
{
  ...
  tt_int_op(128, OP_EQ,
            crypto_pk_public_encrypt(pk2, data1, sizeof(data1),
                                     "Hello whirled.", 15,
                                     PK_PKCS1_OAEP_PADDING));
  tt_int_op(128, OP_EQ,
            crypto_pk_public_encrypt(pk1, data2, sizeof(data1),
                                     "Hello whirled.", 15,
                                     PK_PKCS1_OAEP_PADDING));
  ...
  tt_assert(
      ! crypto_pk_write_private_key_to_filename(pk1, get_fname("pkey1")));
  ...
  tt_int_op(crypto_pk_read_private_key_from_filename(pk2, get_fname("xyzzy")),
            OP_LT, 0);
  tt_assert(
      ! crypto_pk_read_private_key_from_filename(pk2, get_fname("pkey1")));
  tt_int_op(15, OP_EQ,
  ...
  tt_int_op(
      128, OP_EQ,
      crypto_pk_private_sign_digest(pk1, data2, sizeof(data2), data1, 10));
  tt_int_op(20, OP_EQ,
            crypto_pk_public_checksig(pk1, data3, sizeof(data3), data2, 128));
  tt_int_op(0, OP_EQ,
  ...
  for (i = 85; i < 140; ++i) {
    memset(data2, 0, 1024);
    memset(data3, 0, 1024);
    len = crypto_pk_obsolete_public_hybrid_encrypt(
        pk1, data2, sizeof(data2), data1, i, PK_PKCS1_OAEP_PADDING, 0);
    tt_int_op(len, OP_GE, 0);
    len = crypto_pk_obsolete_private_hybrid_decrypt(
        pk1, data3, sizeof(data3), data2, len, PK_PKCS1_OAEP_PADDING, 1);
...
test_crypto_pk_pem_encrypted(void *arg)
{
  ...
  */
  {
    const char *s = "-----BEGIN RSA PUBLIC KEY-----\n"
                    "Proc-Type:4,ENCRYPTED\n"
                    "DEK-Info:des-ede -\n"
                    "\n"
                    "iRqK\n"
...
test_crypto_digests(void *arg)
{
  ...
  tt_assert(r == 0);
  tt_mem_op(hex_str(digest, DIGEST_LEN), OP_EQ, AUTHORITY_SIGNKEY_A_DIGEST,
            HEX_DIGEST_LEN);
...
test_crypto_digest_names(void *arg)
{
  static const struct {
    int a;
    const char *n;
  } names[] = {{DIGEST_SHA1, "sha1"},         {DIGEST_SHA256, "sha256"},
               {DIGEST_SHA512, "sha512"},     {DIGEST_SHA3_256, "sha3-256"},
               {DIGEST_SHA3_512, "sha3-512"}, {-1, NULL}};
  (void)arg;
...
test_crypto_formats(void *arg)
{
  ...
  tt_int_op(-1, OP_EQ,
            base16_decode(data2, 7, data1, 16)); /* dest too short */
...
test_crypto_aes_iv(void *arg)
{
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 4095, plain, 4095);
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted2, 16 + 4095, plain, 4095);
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 1, plain_1, 1);
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 15, plain_15, 15);
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 16, plain_16, 16);
  ...
  encrypted_size =
      crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 17, plain_17, 17);
...
test_crypto_kdf_TAP(void *arg)
{
  ...
  (void)arg;
#define EXPAND(s)                                                     \
  r = crypto_expand_key_material_TAP((const uint8_t *)(s), strlen(s), \
                                     key_material, 100)
...
#define EXPAND(s)                                                \
  r = crypto_expand_key_material_rfc5869_sha256(                 \
      (const uint8_t *)(s), strlen(s), salt, salt_len, m_expand, \
      m_expand_len, key_material, 100)
...
test_crypto_hkdf_sha256_testvecs(void *arg)
{
  ...
  for (i = 0; vecs[i].ikm16; ++i) {
    ...
    int r = crypto_expand_key_material_rfc5869_sha256(
        (const uint8_t *)ikm, ikm_len, (const uint8_t *)salt, salt_len,
        (const uint8_t *)info, info_len, (uint8_t *)okm, okm_len);
    tt_int_op(r, OP_EQ, 0);
...
test_crypto_curve25519_persist(void *arg)
{
  ...
  tt_mem_op(keypair.pubkey.public_key, OP_EQ, keypair2.pubkey.public_key,
            CURVE25519_PUBKEY_LEN);
  tt_mem_op(keypair.seckey.secret_key, OP_EQ, keypair2.seckey.secret_key,
  ...
  cp = content + 32;
  tt_mem_op(keypair.seckey.secret_key, OP_EQ, cp, CURVE25519_SECKEY_LEN);
...
test_crypto_ed25519_simple(void *arg)
{
  ...
  const uint8_t msg[] = "GNU will be able to run Unix programs, "
  ...
  /* Test the string-prefixed sign/checksig functions */
  {
    ...
    tt_int_op(0, OP_EQ,
              ed25519_sign_prefixed(&sig1, msg, msg_len, "always in the mood",
                                    &kp1));
    ...
    tt_int_op(0, OP_EQ,
              ed25519_sign(&manual_sig, (uint8_t *)prefixed_msg,
    ...
    tt_int_op(0, OP_EQ,
              ed25519_checksig_prefixed(&sig1, msg, msg_len,
                                        "always in the mood", &pub1));

    /* Test that checksig with wrong prefix fails. */
    tt_int_op(-1, OP_EQ,
              ed25519_checksig_prefixed(&sig1, msg, msg_len,
                                        "always in the moo", &pub1));
    tt_int_op(-1, OP_EQ,
              ed25519_checksig_prefixed(&sig1, msg, msg_len,
                                        "always in the moon", &pub1));
    tt_int_op(-1, OP_EQ,
              ed25519_checksig_prefixed(&sig1, msg, msg_len,
                                        "always in the mood!", &pub1));
  }
...
test_crypto_ed25519_test_vectors(void *arg)
{
  ...
  for (i = 0; items[i].pk; ++i) {
    ...
    uint8_t *msg;
    size_t msg_len;
    base16_decode((char *)sk_seed, sizeof(sk_seed), items[i].sk, 64);
...
{
  const uint8_t msg[] = "The eyes are not here / There are no eyes here.";
  const int N = 30;
  int i;
  (void)arg;
  ...
  for (i = 0; i < N; ++i) {
    ...
    tt_int_op(0, OP_EQ,
              curve25519_keypair_generate(&curve25519_keypair, i & 1));
    tt_int_op(0, OP_EQ,
              ed25519_keypair_from_curve25519_keypair(&ed25519_keypair, &bit,
                                                      &curve25519_keypair));
    tt_int_op(0, OP_EQ,
    ...
    tt_int_op(0, OP_EQ,
              ed25519_sign(&sig, msg, sizeof(msg), &ed25519_keypair));
    tt_int_op(0, OP_EQ,
              ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey));

    tt_int_op(-1, OP_EQ,
              ed25519_checksig(&sig, msg, sizeof(msg) - 1, &ed25519_pubkey));
    sig.sig[0] ^= 15;
    tt_int_op(-1, OP_EQ,
              ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey));
  }
...
test_crypto_ed25519_blinding(void *arg)
{
  ...
  for (i = 0; i < N; ++i) {
    ...
    tt_int_op(0, OP_EQ,
              ed25519_keypair_blind(&ed25519_keypair_blinded, &ed25519_keypair,
                                    blinding));
    ...
    tt_int_op(0, OP_EQ,
              ed25519_sign(&sig, msg, sizeof(msg), &ed25519_keypair_blinded));

    tt_int_op(
        0, OP_EQ,
        ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey_blinded));

    tt_int_op(
        -1, OP_EQ,
        ed25519_checksig(&sig, msg, sizeof(msg) - 1, &ed25519_pubkey_blinded));
    sig.sig[0] ^= 15;
    tt_int_op(
        -1, OP_EQ,
        ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey_blinded));
...
test_crypto_ed25519_blinding_fail(void *arg)
{
  ...
  retval = base16_decode((char *)pub.pubkey, sizeof(pub.pubkey), badkey,
                         strlen(badkey));
  ...
  retval = base16_decode((char *)pub.pubkey, sizeof(pub.pubkey), goodkey,
                         strlen(goodkey));
  tt_int_op(retval, OP_EQ, sizeof(pub.pubkey));
  retval = ed25519_public_blind(&pub_blinded, &pub, param);
...
#define EQ(a, h) test_memeq_hex((const char *)(a), (h))
    ...
    tt_int_op(sizeof(blinding_param), OP_EQ,
              DECODE(blinding_param, ED25519_BLINDING_PARAMS[i]));
    ...
    tt_int_op(0, OP_EQ, ed25519_public_blind(&blind_pk, &pk, blinding_param));
...
#define FAILURE_MODE_BUFFER_SIZE_I64 (FAILURE_MODE_BUFFER_SIZE / 8)
#define FAILURE_MODE_BUFFER_SIZE_I64_BYTES (FAILURE_MODE_BUFFER_SIZE_I64 * 8)
...
test_crypto_ed25519_validation(void *arg)
{
  ...
  { /* This one is a good key */
    const char goodkey[] =
        "4ba2e44760dff4c559ef3c38768c1c14a8a54740c782c8d70803e9d6e3ad8794";
    retval = base16_decode((char *)pub1.pubkey, sizeof(pub1.pubkey), goodkey,
                           strlen(goodkey));
    tt_int_op(retval, OP_EQ, sizeof(pub1.pubkey));
    tt_int_op(ed25519_validate_pubkey(&pub1), OP_EQ, 0);
  }
...
struct testcase_t crypto_tests[] = {
    ...
    {"aes_AES", test_crypto_aes128, TT_FORK, &passthrough_setup,
     (void *)"aes"},
    {"aes_EVP", test_crypto_aes128, TT_FORK, &passthrough_setup,
     (void *)"evp"},
    {"aes128_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup,
     (void *)"128"},
    {"aes192_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup,
     (void *)"192"},
    {"aes256_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup,
     (void *)"256"},
    CRYPTO_LEGACY(sha),
    CRYPTO_LEGACY(pk),
    ...
    {"pk_invalid_private_key", test_crypto_pk_invalid_private_key, 0, NULL,
     NULL},
    ...
    {"curve25519_impl_hibit", test_crypto_curve25519_impl, 0, NULL,
     (void *)"y"},
    {"curve25516_testvec", test_crypto_curve25519_testvec, 0, NULL, NULL},
    {"curve25519_basepoint", test_crypto_curve25519_basepoint, TT_FORK, NULL,
     NULL},
