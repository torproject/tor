...
server1(int argc, char **argv)
{
  ...
  if (onion_skin_ntor_server_handshake(msg_in, keymap, NULL, node_id, msg_out,
                                       keys, (size_t)keybytes) < 0) {
    fprintf(stderr, "handshake failed");
    result = 2;
    goto done;
  }
...
client2(int argc, char **argv)
{
  ...
  keys = tor_malloc(keybytes);
  hexkeys = tor_malloc(keybytes * 2 + 1);
  if (onion_skin_ntor_client_handshake(&state, msg, keys, keybytes, NULL) <
      0) {
    fprintf(stderr, "handshake failed");
    result = 2;
    goto done;
