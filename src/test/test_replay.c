...
test_replaycache_miss(void *arg)
{
  ...
  result = replaycache_add_and_test_internal(1200, r, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 0);

  /* make sure a different buffer misses as well */
  result = replaycache_add_and_test_internal(1200, NULL, test_buffer_2,
                                             strlen(test_buffer_2), NULL);
  tt_int_op(result, OP_EQ, 0);

  /* poke the bad-parameter error case too */
  result = replaycache_add_and_test_internal(1200, NULL, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 0);
...
test_replaycache_hit(void *arg)
{
  ...
  result = replaycache_add_and_test_internal(1300, r, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 1);

  /* make sure a different buffer misses then hits as well */

  result = replaycache_add_and_test_internal(1200, r, test_buffer_2,
                                             strlen(test_buffer_2), NULL);
  tt_int_op(result, OP_EQ, 0);

  result = replaycache_add_and_test_internal(1300, r, test_buffer_2,
                                             strlen(test_buffer_2), NULL);
  tt_int_op(result, OP_EQ, 1);
...
test_replaycache_age(void *arg)
{
  ...
  result = replaycache_add_and_test_internal(3000, r, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 0);
...
test_replaycache_scrub(void *arg)
{
  ...
  result = replaycache_add_and_test_internal(100, r, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 0);

  result = replaycache_add_and_test_internal(200, r, test_buffer,
                                             strlen(test_buffer), NULL);
  tt_int_op(result, OP_EQ, 1);
...
test_replaycache_future(void *arg)
{
   ...
   */
  result = replaycache_add_and_test_internal(150, r, test_buffer,
                                             strlen(test_buffer), &elapsed);
...
test_replaycache_realtime(void *arg)
{
  ...
  /* This should miss */
  result = replaycache_add_and_test(r, test_buffer, strlen(test_buffer));
  tt_int_op(result, OP_EQ, 0);
  ...
  result = replaycache_add_test_and_elapsed(r, test_buffer,
                                            strlen(test_buffer), &elapsed);
  tt_int_op(result, OP_EQ, 1);
  tt_assert(elapsed >= 0);
  tt_assert(elapsed <= 5);
...
struct testcase_t replaycache_tests[] = {
    REPLAYCACHE_LEGACY(alloc),     REPLAYCACHE_LEGACY(badalloc),
    REPLAYCACHE_LEGACY(free_null), REPLAYCACHE_LEGACY(miss),
    REPLAYCACHE_LEGACY(hit),       REPLAYCACHE_LEGACY(age),
    REPLAYCACHE_LEGACY(elapsed),   REPLAYCACHE_LEGACY(noexpire),
    REPLAYCACHE_LEGACY(scrub),     REPLAYCACHE_LEGACY(future),
    REPLAYCACHE_LEGACY(realtime),  END_OF_TESTCASES};
