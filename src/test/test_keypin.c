...
test_keypin_parse_line(void *arg)
{
  ...
  /* Good line */
  ent =
      keypin_parse_journal_line("aGVyZSBpcyBhIGdvb2Qgc2hhMSE "
  ...
  ent =
      keypin_parse_journal_line("aGVyZSBpcyBhIGdv!2Qgc2hhMSE "
                                "VGhpcyBlZDI1NTE5IHNjb2ZmcyBhdCB0aGUgc2hhMS4");
  tt_ptr_op(ent, OP_EQ, NULL);
...
test_keypin_parse_file(void *arg)
{
  ...
  const char data1[] = "PT09PT09PT09PT09PT09PT09PT0 "
                       "PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT0\n"
                       "TG9yYXggaXBzdW0gZ3J1dnZ1bHU "
                       "cyB0aG5lZWQgYW1ldCwgc25lcmdlbGx5IG9uY2UtbGU\n"
                       "ciBsZXJraW0sIHNlZCBkbyBiYXI "
                       "YmFsb290IHRlbXBvciBnbHVwcGl0dXMgdXQgbGFib3I\n"
                       "ZSBldCB0cnVmZnVsYSBtYWduYSA "
                       "YWxpcXVhLiBVdCBlbmltIGFkIGdyaWNrbGUtZ3Jhc3M\n"
                       "dmVuaWFtLCBxdWlzIG1pZmYtbXU "
                       "ZmZlcmVkIGdhLXp1bXBjbyBsYWJvcmlzIG5pc2kgdXQ\n"
                       "Y3J1ZmZ1bHVzIGV4IGVhIHNjaGw "
                       "b3BwaXR5IGNvbnNlcXVhdC4gRHVpcyBhdXRlIHNuYXI\n"
                       "Z2dsZSBpbiBzd29tZWVzd2FucyA "
                       "aW4gdm9sdXB0YXRlIGF4ZS1oYWNrZXIgZXNzZSByaXA\n"
                       "cHVsdXMgY3J1bW1paSBldSBtb28 "
                       "ZiBudWxsYSBzbnV2di5QTFVHSFBMT1ZFUlhZWlpZLi4\n";
  ...
  const char data2[] =
      "PT09PT09PT09PT09PT09PT09PT0 "
      "PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT0\n"
      "# This is a comment.\n"
      "     \n"
      "QXQgdGhlIGVuZCBvZiB0aGUgeWU "
      "YXIgS3VycmVta2FybWVycnVrIHNhaWQgdG8gaGltLCA\n"
      "IllvdSBoYXZlIG1hZGUgYSBnb28 "
      "ZCBiZWdpbm5pbmcuIiBCdXQgbm8gbW9yZS4gV2l6YXI\n"
      "\n"
      "ZHMgc3BlYWsgdHJ1dGgsIGFuZCA "
      "aXQgd2FzIHRydWUgdGhhdCBhbGwgdGhlIG1hc3Rlcgo\n"
      ...
      "A line too long\n"
      "dGhlIG1lcmUgc3RhcnQgb2Ygd2g "
      "YXQgaGUgbXVzdCBnbyBvb!BsZWFybmluZy4uLi4uLi4\n"
      "ZHMgc3BlYWsgdaJ1dGgsIGFuZCA "
      "aXQgd2FzIHRydWUgdGhhdCBhbGwgdGhlIG1hc3Rlcgo\n"
      "ZHMgc3BlYWsgdHJ1dGgsIGFuZCA "
      "aXQgd2FzIHRydaUgdGhhdCBhbGwgdGhlIG1hc3Rlcgo\n";
  ...
  const char data3[] = "Tm8gZHJhZ29uIGNhbiByZXNpc3Q "
                       "IHRoZSBmYXNjaW5hdGlvbiBvZiByaWRkbGluZyB0YWw";
...
#define ADD(a, b) \
  keypin_check_and_add((const uint8_t *)(a), (const uint8_t *)(b), 0)
...
test_keypin_add_entry(void *arg)
{
  ...
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("gentleman-adventurer", "cloak-and-dagger what's-his-face"));

  tt_int_op(KEYPIN_FOUND, OP_EQ,
            ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
  tt_int_op(KEYPIN_FOUND, OP_EQ,
            ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
  tt_int_op(KEYPIN_FOUND, OP_EQ,
  ...
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("Johnnies-come-lately", "run-of-the-mill root-mean-square"));

  tt_int_op(KEYPIN_MISMATCH, OP_EQ,
            ADD("gentleman-adventurer", "hypersentimental closefistedness"));

  tt_int_op(KEYPIN_MISMATCH, OP_EQ,
            ADD("disestablismentarian", "cloak-and-dagger what's-his-face"));
...
test_keypin_journal(void *arg)
{
  ...
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("king-of-the-herrings", "good-for-nothing attorney-at-law"));
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
  tt_int_op(KEYPIN_FOUND, OP_EQ,
            ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
  ...
  tt_int_op(KEYPIN_FOUND, OP_EQ,
            ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("theatre-in-the-round", "holier-than-thou jack-in-the-box"));
  tt_int_op(KEYPIN_ADDED, OP_EQ,
            ADD("no-deposit-no-return", "across-the-board will-o-the-wisp"));
  tt_int_op(KEYPIN_MISMATCH, OP_EQ,
            ADD("intellectualizations", "salt-and-pepper high-muck-a-muck"));
  keypin_close_journal();
  keypin_clear();
  ...
  tt_int_op(KEYPIN_FOUND, OP_EQ,
            ADD("theatre-in-the-round", "holier-than-thou jack-in-the-box"));
  tt_int_op(KEYPIN_MISMATCH, OP_EQ,
            ADD("counterrevolutionary", "holier-than-thou jack-in-the-box"));
  tt_int_op(KEYPIN_MISMATCH, OP_EQ,
            ADD("no-deposit-no-return", "floccinaucinihilipilificationism"));
  keypin_close_journal();
  ...
  tt_str_op(contents, OP_EQ,
            ...
            "a2luZy1vZi10aGUtaGVycmluZ3M "
            "Z29vZC1mb3Itbm90aGluZyBhdHRvcm5leS1hdC1sYXc\n"
            "eWVsbG93aXNoLXJlZC15ZWxsb3c "
            "c2FsdC1hbmQtcGVwcGVyIGhpZ2gtbXVjay1hLW11Y2s\n"
            "\n"
            "@opened-at 2009-01-04 04:00:00\n"
            "dGhlYXRyZS1pbi10aGUtcm91bmQ "
            "aG9saWVyLXRoYW4tdGhvdSBqYWNrLWluLXRoZS1ib3g\n"
            "bm8tZGVwb3NpdC1uby1yZXR1cm4 "
            "YWNyb3NzLXRoZS1ib2FyZCB3aWxsLW8tdGhlLXdpc3A\n"
            "\n"
...
struct testcase_t keypin_tests[] = {
    TEST(parse_line, 0), TEST(parse_file, TT_FORK), TEST(add_entry, TT_FORK),
    TEST(journal, TT_FORK), END_OF_TESTCASES};
