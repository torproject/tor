...
cert_from_der64(const char *der64)
{
  ...
  derlen = base64_decode((char *)der, der64len, der64, der64len);
  if (derlen >= 0)
...
test_x509_expiration(void *arg)
{
  ...
  tt_assert(tor_tls_cert_is_valid(
      LOG_ERR, cert, cert, now + 365 * 86400 + TOR_X509_PAST_SLOP - 3600, 0));

  tt_assert(!tor_tls_cert_is_valid(LOG_INFO, cert, cert,
                                   now - TOR_X509_FUTURE_SLOP - 3600, 0));
  tt_assert(!tor_tls_cert_is_valid(
      LOG_INFO, cert, cert, now + 365 * 86400 + TOR_X509_FUTURE_SLOP, 0));
...
#define TEST(name)                            \
  {                                           \
#    name, test_x509_##name, TT_FORK, 0, NULL \
  }

struct testcase_t x509_tests[] = {
    TEST(cert_new_failing_digest), TEST(consume_ec_cert),
    TEST(reject_tiny_keys), TEST(expiration), END_OF_TESTCASES};
