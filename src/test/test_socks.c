...
static const struct testcase_setup_t socks_setup = {socks_test_setup,
                                                    socks_test_cleanup};
...
#define ADD_DATA(buf, s) buf_add(buf, s, sizeof(s) - 1)
...
test_socks_4_supported_commands(void *ptr)
{
  ...
  tt_int_op(fetch_from_buf_socks(buf, socks, 1, get_options()->SafeSocks),
            OP_EQ, 1);
  tt_int_op(4, OP_EQ, socks->socks_version);
  tt_int_op(0, OP_EQ, socks->replylen); /* XXX: shouldn't tor reply? */
...
test_socks_client_v4(void *arg)
{
  ...
  tt_int_op(
      1, OP_EQ,
      fetch_from_buf_socks_client(buf, PROXY_SOCKS4_WANT_CONNECT_OK, &reason));
...
test_socks_client_v5_auth(void *arg)
{
  ...
  tt_int_op(1, OP_EQ,
            fetch_from_buf_socks_client(
                buf, PROXY_SOCKS5_WANT_AUTH_METHOD_NONE, &reason));
  tt_ptr_op(reason, OP_EQ, NULL);
  tt_int_op(buf_datalen(buf), OP_EQ, 0);
  ...
  tt_int_op(1, OP_EQ,
            fetch_from_buf_socks_client(
                buf, PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929, &reason));
  tt_ptr_op(reason, OP_EQ, NULL);
  tt_int_op(buf_datalen(buf), OP_EQ, 0);
  ...
  tt_int_op(1, OP_EQ,
            fetch_from_buf_socks_client(buf, PROXY_SOCKS5_WANT_AUTH_RFC1929_OK,
                                        &reason));
  tt_ptr_op(reason, OP_EQ, NULL);
  tt_int_op(buf_datalen(buf), OP_EQ, 0);
...
test_socks_client_v5_connect(void *arg)
{
  ...
  tt_int_op(
      1, OP_EQ,
      fetch_from_buf_socks_client(buf, PROXY_SOCKS5_WANT_CONNECT_OK, &reason));
  tt_ptr_op(reason, OP_EQ, NULL);
  tt_int_op(buf_datalen(buf), OP_EQ, 0);
...
#define S(str) str, (sizeof(str) - 1)
  const struct {
    ...
    size_t len;
      ...
      {PROXY_SOCKS5_WANT_CONNECT_OK, S("\x05\x00\x00\x04"
                                       "abcdefghijklmnop"
                                       "\x00\x05")},
      {PROXY_SOCKS5_WANT_CONNECT_OK, S("\x05\x00\x00\x03\x12"
                                       "gopher.example.com"
                                       "\x00\x05")},
      {PROXY_SOCKS5_WANT_CONNECT_OK, S("\x05\x03\x00\x03\x12"
                                       "gopher.example.com"
                                       "\x00\x05")},
      {PROXY_SOCKS5_WANT_CONNECT_OK, S("\x05\x03\x00\x17")},
  };
  unsigned i, j;
