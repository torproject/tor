...
test_ext_or_write_command(void *arg)
{
  ...
  /* Empty command */
  tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 0x99, NULL, 0), OP_EQ,
            0);
  ...
  /* Medium command. */
  tt_int_op(
      connection_write_ext_or_command(TO_CONN(c1), 0x99, "Wai\0Hello", 9),
      OP_EQ, 0);
  cp = buf_get_contents(TO_CONN(c1)->outbuf, &sz);
  ...
  memset(buf, 'x', 65535);
  tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 0xf00d, buf, 65535),
            OP_EQ, 0);
  cp = buf_get_contents(TO_CONN(c1)->outbuf, &sz);
...
{
  char *reply = NULL, *reply2 = NULL, *client_hash = NULL,
       *client_hash2 = NULL;
  size_t reply_len = 0;
  char hmac1[32], hmac2[32];

  const char client_nonce[32] = "Who is the third who walks alway";
  char server_hash_input[] = "ExtORPort authentication server-to-client hash"
                             "Who is the third who walks alway"
                             "................................";
  char client_hash_input[] = "ExtORPort authentication client-to-server hash"
                             "Who is the third who walks alway"
                             "................................";
...
test_ext_or_cookie_auth_testvec(void *arg)
{
   ...
   */
  test_memeq_hex(reply, "ec80ed6e546d3b36fdfc22fe1315416b"
   ...
   */
  test_memeq_hex(client_hash, "ab391732dd2ed968cd40c087d1b1f25b"
...
static void
ignore_bootstrap_problem(const char *warn, int reason, or_connection_t *conn)
{
  (void)warn;
  (void)reason;
  (void)conn;
...
test_ext_or_handshake(void *arg)
{
  ...
  WRITE("\xff\xf0\x00\x03"
        "ABC",
        7);
  tt_int_op(0, OP_EQ, connection_ext_or_process_inbuf(conn));
  CONTAINS("", 0);
  /* Now let's send a USERADDR command. */
  WRITE("\x00\x01\x00\x0c"
        "1.2.3.4:5678",
        16);
  tt_int_op(0, OP_EQ, connection_ext_or_process_inbuf(conn));
...
struct testcase_t extorport_tests[] = {
    ...
    {"cookie_auth_testvec", test_ext_or_cookie_auth_testvec, TT_FORK, NULL,
     NULL},
