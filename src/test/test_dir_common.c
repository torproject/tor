...
networkstatus_t *dir_common_add_rs_and_parse(
    networkstatus_t *vote, networkstatus_t **vote_out,
    vote_routerstatus_t *(*vrs_gen)(int idx, time_t now),
    crypto_pk_t *sign_skey, int *n_vrs, time_t now, int clear_rl);
...
dir_common_add_rs_and_parse(networkstatus_t *vote, networkstatus_t **vote_out,
                            vote_routerstatus_t *(*vrs_gen)(int idx,
                                                            time_t now),
                            ...
                            int clear_rl)
{
  ...
  *vote_out = NULL;
  ...
  do {
    ...
    if (vrs) {
      ...
      router_added = router_add_to_routerlist(
          dir_common_generate_ri_from_rs(vrs), &msg, 0, 0);
  ...
  *n_vrs = idx;
  ...
  tt_assert(v_text);
  *vote_out = networkstatus_parse_vote_from_string(v_text, strlen(v_text),
                                                   NULL, NS_TYPE_VOTE);
 ...
 */
int
dir_common_construct_vote_1(
    networkstatus_t **vote, authority_cert_t *cert, crypto_pk_t *sign_skey,
    vote_routerstatus_t *(*vrs_gen)(int idx, time_t now),
    networkstatus_t **vote_out, int *n_vrs, time_t now, int clear_rl)
{
  ...
  smartlist_split_string(
      (*vote)->known_flags,
      "Authority Exit Fast Guard Running Stable V2Dir Valid", 0,
  ...
  *n_vrs = 0;
  /* add routerstatuses */
  if (! dir_common_add_rs_and_parse(*vote, vote_out, vrs_gen, sign_skey, n_vrs,
                                    now, clear_rl))
    return -1;
    ...
    networkstatus_t **vote_out, int *n_vrs, time_t now, int clear_rl)
{
  ...
  smartlist_split_string((*vote)->net_params,
                         "bar=2000000000 circuitwindow=20", NULL, 0, 0);
  /* add routerstatuses */
  /* dump the vote and try to parse it. */
  dir_common_add_rs_and_parse(*vote, vote_out, vrs_gen, sign_skey, n_vrs, now,
                              clear_rl);
    ...
    networkstatus_t **vote_out, int *n_vrs, time_t now, int clear_rl)
{
  ...
  smartlist_add((*vote)->voters, voter);
  (*vote)->cert = authority_cert_dup(cert);
  smartlist_split_string((*vote)->net_params, "circuitwindow=80 foo=660", NULL,
                         0, 0);
  /* add routerstatuses */
  /* dump the vote and try to parse it. */
  dir_common_add_rs_and_parse(*vote, vote_out, vrs_gen, sign_skey, n_vrs, now,
