...
test_tortls_tor_tls_get_error(void *data)
{
  ...
  tor_tls_get_error(tls, 0, 0, (const char *)"in unit test", LOG_WARN,
                    LD_GENERAL);
...
test_tortls_double_init(void *arg)
{
  ...
  int r = tor_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER, pk1, pk2, 86400);
  tt_int_op(r, OP_EQ, 0);

  r = tor_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER, pk2, pk1, 86400);
  tt_int_op(r, OP_EQ, 0);
