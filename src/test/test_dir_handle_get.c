...
test_dir_handle_get_bad_request(void *data)
{
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, NULL,
                      NULL, 1, 0);
...
test_dir_handle_get_v1_command_not_found(void *data)
{
  ...
  /* V1 path */
  tt_int_op(directory_handle_command_get(conn, GET("/tor/"), NULL, 0), OP_EQ,
            0);
...
test_dir_handle_get_v1_command(void *data)
{
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, body_len + 1, 0);
...
test_dir_handle_get_robots_txt(void *data)
{
  ...
  tt_int_op(
      directory_handle_command_get(conn, GET("/tor/robots.txt"), NULL, 0),
      OP_EQ, 0);
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, 29, 0);
    ...
    void *data)
{
  ...
  tt_int_op(directory_handle_command_get(
                conn, RENDEZVOUS2_GET("invalid-desc-id"), NULL, 0),
            OP_EQ, 0);
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, NULL,
                      NULL, 1, 0);
...
test_dir_handle_get_rendezvous2_on_encrypted_conn_not_well_formed(void *data)
{
  ...
  // TODO: this cant be reached because rend_valid_descriptor_id() prevents
...
#define RECENT_TIME -10
  ...
  tt_int_op(rend_cache_store_v2_desc_as_dir(desc_holder->desc_str), OP_EQ, 0);
...
test_dir_handle_get_micro_d(void *data)
{
  ...
  base64_encode_nopad(digest_base64, sizeof(digest_base64), (uint8_t *)digest,
                      DIGEST256_LEN);
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, strlen(microdesc) + 1, 0);
...
test_dir_handle_get_server_descriptors_all(void *data)
{
  ...
  // TODO: Is this a BUG?
  // This is what should be expected: tt_int_op(body_used, OP_EQ,
  // strlen(body));
  tt_int_op(body_used, OP_EQ,
            mock_routerinfo->cache_info.signed_descriptor_len);

  tt_str_op(body, OP_EQ,
...
test_dir_handle_get_server_descriptors_authority(void *data)
{
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, strlen(TEST_DESCRIPTOR) + 1, 0);
...
#define HEX2 "Deadbeef99999991111119999911111111f00ba4"
  const char *hex_digest =
      hex_str(mock_routerinfo->cache_info.identity_digest, DIGEST_LEN);
...
test_dir_handle_get_server_descriptors_d(void *data)
{
  ...
  const char *hex_digest =
      hex_str(router->cache_info.signed_descriptor_digest, DIGEST_LEN);
  ...
  // which is smaller than that by annotation_len bytes
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, router->cache_info.signed_descriptor_len + 1,
                      0);
...
test_dir_handle_get_server_keys_all(void *data)
{
  ...
  base16_decode(ds->v3_identity_digest, DIGEST_LEN, TEST_CERT_IDENT_KEY,
                HEX_DIGEST_LEN);
  tt_int_op(
      0, OP_EQ,
      trusted_dirs_load_certs_from_string(
          TEST_CERTIFICATE, TRUSTED_DIRS_CERTS_SRC_DL_BY_ID_DIGEST, 1, NULL));
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, strlen(TEST_CERTIFICATE) + 1, 0);
...
test_dir_handle_get_server_keys_sk(void *data)
{
  ...
  conn = new_dir_conn();
  char req[71];
  tor_snprintf(req, sizeof(req), GET("/tor/keys/sk/%s"), TEST_SIGNING_KEY);
  tt_int_op(directory_handle_command_get(conn, req, NULL, 0), OP_EQ, 0);
...
test_dir_handle_get_server_keys_fpsk(void *data)
{
  ...
  char req[115];
  tor_snprintf(req, sizeof(req), GET("/tor/keys/fp-sk/%s-%s"),
...
#define NETWORK_STATUS "some network status string"
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn,
                GET("/tor/status-vote/current/consensus-ns/" HEX1 "+" HEX2),
                NULL, 0));
...
test_dir_handle_get_status_vote_current_consensus_ns_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/current/consensus-ns"), NULL, 0));
...
#define NETWORK_STATUS "some network status string"
  ...
  tt_int_op(
      0, OP_EQ,
      directory_handle_command_get(
          conn, GET("/tor/status-vote/current/consensus-microdesc"), NULL, 0));
...
#define NETWORK_STATUS "some network status string"
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/current/consensus"), NULL, 0));
...
int NS(geoip_get_country_by_addr)(const tor_addr_t *addr)
{
  (void)addr;
...
#define NETWORK_STATUS "some network status string"
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body,
                      body_len, strlen(NETWORK_STATUS) + 7, 0);
...
test_dir_handle_get_status_vote_current_consensus_ns(void *data)
{
  ...
  compress_method_t compression =
      detect_compression_method(comp_body, comp_body_used);
  tt_int_op(ZLIB_METHOD, OP_EQ, compression);

  tor_uncompress(&body, &body_used, comp_body, comp_body_used, compression, 0,
                 LOG_PROTOCOL_WARN);
...
test_dir_handle_get_status_vote_current_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/current/" HEX1), NULL, 0));
...
status_vote_current_d_test(char **header, char **body, size_t *body_l)
{
  ...
  tt_int_op(
      0, OP_EQ,
      directory_handle_command_get(
          conn, GET("/tor/status-vote/current/d/" VOTE_DIGEST), NULL, 0));

  fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body,
                      body_l, strlen(VOTE_BODY_V3) + 1, 0);
  tt_assert(header);
...
status_vote_next_d_test(char **header, char **body, size_t *body_l)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/d/" VOTE_DIGEST), NULL, 0));
...
test_dir_handle_get_status_vote_d(void *data)
{
  ...
  int status_out = 0;
  struct pending_vote_t *pv =
      dirvote_add_vote(VOTE_BODY_V3, &msg_out, &status_out);
  tt_assert(pv);
...
test_dir_handle_get_status_vote_next_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/" HEX1), NULL, 0));
...
status_vote_next_consensus_test(char **header, char **body, size_t *body_used)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/consensus"), NULL, 0));
...
test_dir_handle_get_status_vote_current_authority_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/current/authority"), NULL, 0));
...
test_dir_handle_get_status_vote_next_authority_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/authority"), NULL, 0));
...
test_dir_handle_get_status_vote_next_bandwidth_not_found(void *data)
{
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/bandwdith"), NULL, 0));
...
NS_DECL(const char *, dirvote_get_pending_consensus,
        (consensus_flavor_t flav));

const char *NS(dirvote_get_pending_consensus)(consensus_flavor_t flav)
{
  (void)flav;
                                           ...
                                           size_t *body_used)
{
  ...
  tt_int_op(
      0, OP_EQ,
      directory_handle_command_get(
          conn, GET("/tor/status-vote/next/consensus-signatures"), NULL, 0));
...
const char *NS(dirvote_get_pending_detached_signatures)(void)
{
...
test_dir_handle_get_status_vote_next_authority(void *data)
{
  ...
  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, strlen(VOTE_BODY_V3) + 1, 0);
...
test_dir_handle_get_status_vote_next_bandwidth(void *data)
{
  ...
  mock_options->V3BandwidthsFile =
      tor_strdup(get_fname_rnd("V3BandwidthsFile"));
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/bandwidth"), NULL, 0));

  fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body,
                      &body_used, strlen(content) + 1, 0);
  ...
  tt_int_op(0, OP_EQ,
            directory_handle_command_get(
                conn, GET("/tor/status-vote/next/bandwidth.z"), NULL, 0));
