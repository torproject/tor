...
test_md_cache(void *data)
{
  ...
  added = microdescs_add_to_cache(mc, test_md1, NULL, SAVED_NOWHERE, 0, time1,
                                  NULL);
  ...
  added = microdescs_add_to_cache(mc, test_md2, NULL, SAVED_NOWHERE, 0, time2,
                                  wanted);
  ...
  added =
      microdescs_add_to_cache(mc, test_md3, NULL, SAVED_NOWHERE, 0, -1, NULL);
...
test_md_parse(void *arg)
{
  ...
  smartlist_t *mds = microdescs_parse_from_string(MD_PARSE_TEST_DATA, NULL, 1,
                                                  SAVED_NOWHERE, invalid);
  ...
  test_memeq_hex(md->digest, "54bb6d733ddeb375d2456c79ae103961"
  ...
  test_memeq_hex(md->digest, "53f740bd222ab37f19f604b1d3759aa6"
                             "5eff1fbce9ac254bd0fa50d4af9b1bae");
  tt_assert(! md->exit_policy);

  md = smartlist_get(mds, 8);
  test_memeq_hex(md->digest, "a0a155562d8093d8fd0feb7b93b7226e"
                             "17f056c2142aab7a4ea8c5867a0376d5");
  tt_assert(md->onion_curve25519_pkey);

  md = smartlist_get(mds, 10);
  test_memeq_hex(md->digest, "409ebd87d23925a2732bd467a92813c9"
...
test_md_parse_id_ed25519(void *arg)
{
  ...
  mds = microdescs_parse_from_string(GOOD_MD, NULL, 1, SAVED_NOWHERE, NULL);
  tt_assert(mds);
  ...
  mds = microdescs_parse_from_string(DUPLICATE_KEY, NULL, 1, SAVED_NOWHERE,
                                     NULL);
  tt_assert(mds);
  ...
  mds = microdescs_parse_from_string(BOGUS_KEY, NULL, 1, SAVED_NOWHERE, NULL);
  tt_assert(mds);
  tt_int_op(smartlist_len(mds), OP_EQ, 0); // no entries.
...
#define ADD(hex)                                                  \
  do {                                                            \
    tt_int_op(sizeof(buf), OP_EQ,                                 \
              base16_decode(buf, sizeof(buf), hex, strlen(hex))); \
  ...
  added = microdescs_add_to_cache(mc, MD_PARSE_TEST_DATA, NULL, SAVED_NOWHERE,
                                  0, time(NULL), wanted);
...
test_md_corrupt_desc(void *arg)
{
  ...
  sl = microdescs_add_to_cache(get_microdesc_cache(), cp, cp + strlen(cp),
