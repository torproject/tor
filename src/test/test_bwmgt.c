...
test_bwmgt_dir_conn_global_write_low(void *arg)
{
  ...
  tor_addr_parse(&conn->addr, "1.1.1.1");
  ret = connection_dir_is_global_write_low(conn, INT_MAX, 2);
  tt_int_op(ret, OP_EQ, 1);

  /* Now, lets try with a connection address from moria1. It should always
   * pass even though our limit is too low. */
  tor_addr_parse(&conn->addr, "128.31.0.39");
  ret = connection_dir_is_global_write_low(conn, INT_MAX, 2);
  tt_int_op(ret, OP_EQ, 0);
  ...
  tor_addr_parse(&conn->addr, "1.2.3.4");
  ret = connection_dir_is_global_write_low(conn, INT_MAX, 2);
  tt_int_op(ret, OP_EQ, 0);
