...
test_bwmgt_token_buf_refill(void *arg)
{
  ...
  tt_int_op(
      0, OP_EQ,
      token_bucket_rw_refill(&b, START_TS + BW_SEC * 3 / 2 + BW_SEC * 30));
  ...
  tt_int_op(
      0, OP_EQ,
      token_bucket_rw_refill(&b, START_TS + BW_SEC * 3 / 2 + BW_SEC * 60));
  tt_int_op(b.read_bucket.bucket, OP_EQ, b.cfg.burst);
  tt_uint_op(b.last_refilled_at_timestamp, OP_EQ,
  ...
  tt_int_op(
      1, OP_EQ,
      token_bucket_rw_refill(&b, START_TS + BW_SEC * 3 / 2 + BW_SEC * 61));
  tt_int_op(
      0, OP_EQ,
      token_bucket_rw_refill(&b, START_TS + BW_SEC * 3 / 2 + BW_SEC * 62));
...
struct testcase_t bwmgt_tests[] = {
    BWMGT(token_buf_init),   BWMGT(token_buf_adjust),  BWMGT(token_buf_dec),
    BWMGT(token_buf_refill), BWMGT(token_buf_helpers), END_OF_TESTCASES};
