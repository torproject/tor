...
static int netlink_query(tor_socket_t fd, int severity, void *req,
                         size_t reqlen, netlink_msg_fn msgfn, void *ctx);

static int netlink_get_external_ifindex(tor_socket_t fd, int severity,
                                        sa_family_t family);
static int netlink_parse_getroute_msg(void *ctx, struct nlmsghdr *msg);

static int netlink_get_address_ifindex(tor_socket_t fd, int severity,
                                       sa_family_t family, int ifindex,
                                       tor_addr_t *addr);
static int netlink_parse_getaddr_msg(void *ctx, struct nlmsghdr *msg);
...
/** Set <b>addr<b> to the IP Address (if any) of whatever interface connects
 * to the Internet.  This address should only be used in checking whether our
                                ...
                                tor_addr_t *addr))
{
  ...
  if (fd < 0) {
    ...
    tor_log(severity, LD_NET, "unable to create socket: %s",
  ...
  /* Query the kernel for the routing table, to get the interface index for the
              ...
              netlink_msg_fn msgfn, void *ctx)
{
   ...
   *
   * Note: If the socket is ever used for multiple simultanious queries, or
   * as an async event notification system, all of this will break unless
   * requests contain sequence numbers, and the response processor is
   * updated to care about said sequence numbers.
   */
  do {
    ...
    if (msg.msg_flags & MSG_TRUNC) {
       ...
       * truncated data will still sit in the queue.  It's worth noting that
       * a lot of other userland tools will break if this condition occurs.
       */
